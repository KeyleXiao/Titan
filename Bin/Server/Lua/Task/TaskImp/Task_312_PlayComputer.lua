...
-- 人机我和你
-------------------------------------------------------------------
Task_312_PlayComputer = TaskFramePlayDotaGame:new
{
}
...
----------------------------------------------------------------------------
function Task_312_PlayComputer:OnEventPlayDotaGame(tGameData)
	...
	if tGameData.nParam1 == nil or tGameData.nParam2 == nil then
		return
	end
	
	-- 判断队友是否包含发布者
	local tTeamMateID = split_string(tGameData.nParam1, ";", tonumber)
	if tTeamMateID == nil or table_count(tTeamMateID) <= 0 then
		return
	end

	local bFind = false
	for	_, nTeamMateID in pairs(tTeamMateID) do
		if nTeamMateID == self.publisher_id then
			bFind = true
			break
		end
	end
