...
string CSchemeHeroCard::getUBBNameByCardID( int nCardID )
{
	SHeroCardScheme* pSHeroCardScheme = getByCardID(nCardID);

    return getUBBNameBySHeroCardScheme( pSHeroCardScheme );
}

string CSchemeHeroCard::getUBBNameBySHeroCardScheme( SHeroCardScheme* pSHeroCardScheme )
{
    if(pSHeroCardScheme == NULL)
    {
        return "";
    }
	ISchemeHeroCardGradeColor *pSchemeHeroCardGradeColor = gClientGlobal->getSchemeCenter()->getSchemeHeroCardGradeColor();
	if (pSchemeHeroCardGradeColor == NULL)
    {
         return "";
    }
	SHeroCardGradeColorScheme *pCardColorScheme = pSchemeHeroCardGradeColor->getByCardGrade(pSHeroCardScheme->nCardGrade);
    if(pCardColorScheme == NULL)
    {
        return "";
    }
    
    char szCardUBBName[CARD_NAME_MAXSIZE] = {0};
	ssprintf_s(szCardUBBName, sizeof(szCardUBBName), "[%s]%s[/%s]", pCardColorScheme->szCardColor, pSHeroCardScheme->szCardName, pCardColorScheme->szCardColor);

	std::string  strCardUBBname(szCardUBBName);
	return strCardUBBname;
}
