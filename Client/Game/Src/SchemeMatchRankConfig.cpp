...
int CSchemeMatchRankConfig::cmpRankGrade(int eMatchTypeID, int nPreScore, int nCurScore)
{
	if (nPreScore == nCurScore)
		return 0;

	SMatchRankConfigSchemeInfo* pPreInfo = getShemeInfoByTypeAndScore(eMatchTypeID, nPreScore);
	if (pPreInfo == NULL)
		return 0;
	SMatchRankConfigSchemeInfo* pCurInfo = getShemeInfoByTypeAndScore(eMatchTypeID, nCurScore);
	if (pCurInfo == NULL)
		return 0;

	return pCurInfo->nGradeID - pPreInfo->nGradeID;
}
