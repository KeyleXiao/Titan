...
#define MONSTER_LOAD_SKILL(index)	\
	...
	for (int nRow=0; nRow<nRecordCount; ++nRow)
	{
        ...
        // 小地图图标ID
        item.nMinimapIconID = pCSVReader->GetInt(nRow, MONSTER_COL_MINIMAPICONID, 0);
        ...
        pCSVReader->GetString(nRow, MONSTER_COL_COLOR, szBgColor, nLen);
        char* pEnd = NULL;
        item.nColor = strtol(szBgColor, &pEnd, 16);

		//怪物血条样式
		item.nBloodStyle = pCSVReader->GetInt(nRow, MONSTER_COL_BLOODSTYLE, 0);
