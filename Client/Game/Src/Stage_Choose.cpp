    ...
    case GVIEWCMD_USER_CREATE_PLAYER:       // 创建角色
        {
            ...
            msg.nHeroID = pCmd->heroid;     // 英雄ID
            msg.nSkinID = pCmd->skinid;     // 皮肤ID
            msg.bSex    = pCmd->sex;        // 性别
            ...
            if ( msg.nSkinID==0 )
            {
                ...
                if(pSchemeCenter)
                {
                    ...
                    if(pPersonScheme)
                    {
                        ...
                        if ( pItem)
                        {
                            msg.nSkinID = pItem->nDefaultSkinID;
                            WarningLn("Create Player use default skinid="<< msg.nSkinID );
                        }
                    }
                }
            }
...
void Stage_Choose::onMsgLoginNotifyClient(void * data, size_t len)
{
    ...
    SMsgLoginServer_Message* msg = (SMsgLoginServer_Message*)data;
    if ( len != sizeof(SMsgLoginServer_Message) + msg->wMsgLen)
    {
        ErrorLn(__FUNCTION__ << "len = " << sizeof(SMsgLoginServer_Message) << ", wMsgLen = " << msg->wMsgLen );
        return;
    }
    char* pContent = (char*)(msg+1);
    if(pContent == NULL)
    {
        return;
    }
