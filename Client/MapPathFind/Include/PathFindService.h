...
public:
    ...
    //chf数组数据 + config数据
    rcCompactHeightfield*  m_chfList;		//每个地图一个，service释放的时候不需要释放【sizeof(rcCompactHeightfield) * tileCount】
    NvBuildConfig*  m_chfConfig;		//每个地图一个，service释放的时候不需要释放【sizeof(NvBuildConfig)】
private:
	// 基于凸多边形块的导航网格
	// DetourNavMesh.h
...
public:
    ...
    bool			FindNearestNavmeshPoint(Vector3 const* pVec3Point, rcnNavmeshPoint& rcnNP);
...
private:
	...
	dtStatus	findPathPolygonsInSurface(Vector3 const* pvec3Start,Vector3 const* pvec3End);

    // 3D寻路，即通常意义上的寻路，寻找从路径起始点到终点的多边形路径
	dtStatus	findPathPolygons( Vector3 const* pvec3Start, Vector3 const* pvec3End , unsigned int nFlag);

    // 将从起点到终点连接的多边形，转化为可直接行走的线段（路径）
    bool			findStraightPath(bool isPart);
