...
namespace Effect
{
    ...
    public class EffectUtily 
    {
        ...
        public static void SetEffectColliderLayer(int layer,  ICollection<Collider> effectColliders)
        {

            if (effectColliders == null || effectColliders.Count <= 0)
            {
                return;
            }

            foreach (Collider col in effectColliders)
            {
        ...
        /// <param name="mat"></param>
        /// <returns></returns>
        public static bool IsStandrad(Material mat)
        {
            if (!mat)
            {
                return false;
            }
            return mat.shader.name.Contains("Standard");
        }
    }
}
