...
namespace DG.DOTweenEditor.Core
{
    public static class EditorUtils
    {
        ...
        public static T ConnectToSourceAsset<T>(string adbFilePath, bool createIfMissing = false) where T : ScriptableObject
        {
            ...
            T source = UnityEditor.AssetDatabase.LoadAssetAtPath<T>(adbFilePath);//(T)Resources.LoadAssetAtPath(adbFilePath, typeof(T));
            if (source == null) {
                // Source changed (or editor file was moved from outside of Unity): overwrite it
                CreateScriptableAsset<T>(adbFilePath);
                source = UnityEditor.AssetDatabase.LoadAssetAtPath<T>(adbFilePath);//(T)Resources.LoadAssetAtPath(adbFilePath, typeof(T));
            }
            return source;
        }
