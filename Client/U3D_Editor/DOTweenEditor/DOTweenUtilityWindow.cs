...
namespace DG.DOTweenEditor
{
    ...
    class DOTweenUtilityWindow : EditorWindow
    {
        ...
        static readonly Vector2 _WinSize = new Vector2(300,400);
        ...
        public void OnEnable()
        {
            ...
            if (_headerImg == null) {
                _headerImg = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/" + EditorUtils.editorADBDir + "Imgs/Header.jpg");//Resources.LoadAssetAtPath("Assets/" + EditorUtils.editorADBDir + "Imgs/Header.jpg", typeof(Texture2D)) as Texture2D;
                if(_headerImg != null)
                {
                    ...
                    _footerImg = UnityEditor.AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/" + EditorUtils.editorADBDir + (EditorGUIUtility.isProSkin ? "Imgs/Footer.png" : "Imgs/Footer_dark.png"));//Resources.LoadAssetAtPath("Assets/" + EditorUtils.editorADBDir + (EditorGUIUtility.isProSkin ? "Imgs/Footer.png" : "Imgs/Footer_dark.png"), typeof(Texture2D)) as Texture2D;
        ...
        public void OnGUI()
        {
            ...
            if (Application.isPlaying) {
                ...
                Rect areaRect = new Rect(0, 0, _WinSize.x, 30);
        ...
        void DrawPreferencesGUI()
        {
            ...
            GUILayout.Space(8);
            _src.SettingConfigurationType = (DOTweenSettings.ConfigurationType)EditorGUILayout.EnumPopup("配置表类型", _src.SettingConfigurationType);
            GUILayout.Space(8);
            _src.useSafeMode = EditorGUILayout.Toggle("Safe Mode", _src.useSafeMode);

            if (_src.SettingConfigurationType == DOTweenSettings.ConfigurationType.ECT_Custom)
            {
            ...
            else if(_src.SettingConfigurationType == DOTweenSettings.ConfigurationType.ECT_Debug)
            {
                _src.showUnityEditorReport = true;
                _src.logBehaviour = LogBehaviour.Verbose;
            }
            GUILayout.Space(8);
            ...
            _src.defaultMaxTweenerCapacity = EditorGUILayout.IntField("默认最大Tweener容量", _src.defaultMaxTweenerCapacity);
            _src.defaultMaxSequences = EditorGUILayout.IntField("默认最大Sequences容量", _src.defaultMaxSequences);
        ...
        {
            if (_src == null) {
                //根据项目把路径移到Resources/DOTween下
                string srcDir = EditorUtils.assetsPath + EditorUtils.pathSlash + "Resources"+ EditorUtils.pathSlash + "DOTween";
                
                if (!Directory.Exists(srcDir))
                {
                    AssetDatabase.CreateFolder("Assets/Resources", "DOTween");
                }
                ...
                string legacySrcDir = EditorUtils.dotweenDir + "Resources"+ EditorUtils.pathSlash;
