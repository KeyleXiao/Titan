...
namespace UIWidgets
{ 
    [CustomEditor(typeof(UTooltipTrigger)), CanEditMultipleObjects]
    public class UTooltipTriggerEditor : Editor
    {
    ...
    [CustomEditor(typeof(TooltipStyle)), CanEditMultipleObjects]
    public class UTooltipStyleEditor : Editor
    {
        private SerializedProperty tooltipID;
        public void OnEnable()
        {
            serializedObject.Update();

            tooltipID = serializedObject.FindProperty("m_nTooltipID");
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            TooltipStyle tooltipstyle = target as TooltipStyle;

            tooltipID.intValue = (int)((USpeedUI.ETooltipStyle)EditorGUILayout.EnumPopup(new GUIContent("类型ID"), (USpeedUI.ETooltipStyle)tooltipID.intValue));

            if (GUI.changed)
                EditorUtility.SetDirty(tooltipstyle);
            serializedObject.ApplyModifiedProperties();
        }
    }
}
