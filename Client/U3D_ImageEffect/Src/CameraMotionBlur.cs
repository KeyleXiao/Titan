...
public class CameraMotionBlur : ImageEffectNode 
{
	...
	public override bool ImageEffect_RenderImage(RenderTexture source, RenderTexture destination, RenderBuffer depthBuffer)
    {
		...
		if (filterType == MotionBlurFilter.CameraMotion)
		{
			...
			// pitch (vertical)
			farHeur = (Vector3.Angle (transform.up, prevFrameUp) / m_Camera.fieldOfView) * (source.width * 0.75f);
			...
			farHeur = (Vector3.Angle (transform.forward, prevFrameForward) / m_Camera.fieldOfView) * (source.width * 0.75f);
			...
			if (preview) // crude approximation
				motionBlurMaterial.SetVector ("_BlurDirectionPacked", new Vector4 (previewScale.y, previewScale.x, 0.0f, previewScale.z) * 0.5f * m_Camera.fieldOfView);
			else
	...
	Camera GetTmpCam ()  {
		...
		tmpCam.transform.position = m_Camera.transform.position;
		tmpCam.transform.rotation = m_Camera.transform.rotation;
		tmpCam.transform.localScale = m_Camera.transform.localScale;
		Camera temp = tmpCam.GetComponent<Camera>();
        temp.CopyFrom(m_Camera);

        temp.enabled = false;
        temp.depthTextureMode = DepthTextureMode.None;
        temp.clearFlags = CameraClearFlags.Nothing;
