...
\code
	...
	public void LateUpdate () {
		ik.solver.leftHandEffector.SetPosition( something; // Set the left hand effector position to a point in world space. This has no effect if the effector's positionWeight is 0.
...
<b>Component variables</b>
	- <b>target</b> - the reference of direction from the trigger to the object, that will determine the default direction with 0 angle offset from the position of the character to the interaction object. You can in most cases assign the InteractionObject itself as the Target.
	- <b>ranges</b> - define the angular and positional range of the character relative to the Interaction Object, in which the character is capable of interaction.
	- <b>interactions</b> - the definitions of interactions that will be called on InteractionSystem.TriggerInteraction.
	- <b>effectors</b> - determine the effectors that will be used for this interaction.
	- <b>positionOffset</b> - the position offset of this range from the center of the collider.
	- <b>orbit</b> - if checked, the character's position for that range is valid from any angle of approach.
	- <b>maxDistance</b> - maximum distance of the character from the center of the range. This is visualized with a circle in the Scene View. Make sure the size of the trigger collider is bigger than maxDistance.
	- <b>angleOffset</b> - angular offset of this range.
	- <b>maxAngle</b> - maximum angle of the character's forward from the direction of the range.
