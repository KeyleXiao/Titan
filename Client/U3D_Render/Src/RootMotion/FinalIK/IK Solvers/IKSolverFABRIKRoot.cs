...
namespace RootMotion.FinalIK {
	...
	public class IKSolverFABRIKRoot : IKSolver {
		...
		public override bool IsValid(bool log) {
			if (chains.Length == 0) {
				if (log) LogWarning("IKSolverFABRIKRoot contains no chains.");
				return false;
			}
			...
			foreach (FABRIKChain chain in chains) {
				...
				} else if (!chain.IsValid(null)) return false;
			}

			for (int i = 0; i < chains.Length; i++) {
				for (int c = 0; c < chains.Length; c++) {
					if (i != c && chains[i].ik == chains[c].ik) {
						if (log) LogWarning(chains[i].ik.name + " is represented more than once in IKSolverFABRIKRoot chain.");
						return false;
					}
				}
			}
			...
			for (int i = 0; i < chains.Length; i++) {
				for (int c = 0; c < chains[i].children.Length; c++) {
					...
					if (childIndex < 0) {
						if (log) LogWarning(chains[i].ik.name + "IKSolverFABRIKRoot chain at index " + i + " has invalid children array. Child index is < 0.");
						return false;
					}

					if (childIndex == i) {
						if (log) LogWarning(chains[i].ik.name + "IKSolverFABRIKRoot chain at index " + i + " has invalid children array. Child index is referencing to itself.");
						return false;
					}

					if (childIndex >= chains.Length) {
						if (log) LogWarning(chains[i].ik.name + "IKSolverFABRIKRoot chain at index " + i + " has invalid children array. Child index > number of chains");
						return false;
					}
						...
						if (childIndex == o) {
							for (int n = 0; n < chains[o].children.Length; n++) {
								if (chains[o].children[n] == i) {
									if (log) LogWarning("Circular parenting. " + chains[o].ik.name + " already has " + chains[i].ik.name + " listed as it's child.");
									return false;
								}
							}
						}
					...
					for (int n = 0; n < chains[i].children.Length; n++) {
						if (c != n && chains[i].children[n] == childIndex) {
							if (log) LogWarning("Chain number " + childIndex + " is represented more than once in the children of " + chains[i].ik.name);
							return false;
						}
					}
