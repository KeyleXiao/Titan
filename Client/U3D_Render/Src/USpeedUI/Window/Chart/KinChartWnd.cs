...
{
    public class KinChartWnd : UIBaseSubWnd<KinChartWndView>
    {
        public override int GetWndType()
        {
            return (int)EMChartSubWndType.EMWNDTYPE_KIN;
        }

        public override string GetPrefabPath()
        {
            return UPath.UStaticState + "/Chart/KinChartView";
        }
        ...
        public override bool LoadView()
        {
            ...
            ViewEventHelper.Instance.SendCommand(GameLogicDef.GVIEWCMD_OPEN_ORDER_KIN);
    ...
    public class KinChartWndView : UIBaseSubWndView
    {
        ...
        public void RefreshData()
        {
            ...
            CalculatePageState();
            UChartRankingMsgData data = new UChartRankingMsgData(LogicDataCenter.chartDataManager.KinChartList.PersonalRank);
        ...
        public void OnSearchBtnClick()
        {
            ...
            LogicDataCenter.chartDataManager.SearchChartInfo(EMChartType.EMCHART_KIN, SearchField.text);
        }
        ...
        {
            List<cmd_entity_kin_order_info> dataList = new List<cmd_entity_kin_order_info>();
            if (bIsQuery)
                dataList = LogicDataCenter.chartDataManager.KinChartList.tmpDataList;
            else
            {
                dataList = LogicDataCenter.chartDataManager.KinChartList.AllDataList;
                if (LogicDataCenter.chartDataManager.KinChartList.PersonalRank % ContainerFrame.childCount == 0)
                {
                    m_RankingPage = LogicDataCenter.chartDataManager.KinChartList.PersonalRank / ContainerFrame.childCount;
                    m_RankingPage--;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (trs != null)
                {
                    KinChartItem item = trs.GetComponent<KinChartItem>();
                    if (item != null)
                    {
                        item.Clear();
    ...
    public class KinChartItem : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        ...
        public void SetData(cmd_entity_kin_order_info info)
        {
            index.text = info.nOrderID.ToString();

            kinName.text = info.szKinName;
            ...
            if (!U3D_Render.EntityView.isNullOrEmpty(EntityFactory.MainHeroView))
            {
                ...
                {
                    if (EntityFactory.MainHeroView.Property.GetNumProp(ENTITY_PROPERTY.PROPERTY_KIN) == info.dwKinID)
                    {
                        bSelfItem = true;
