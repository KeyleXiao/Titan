...
{
    public class MentorshipMasterListWnd : UIBaseSubWnd<MentorshipMasterListWndView>
    {
        // 获取ID
        public override int GetWndType()
        {
            return (int)MentorshipSubWndType.WND_MENTORSHIP_MASTERISSUE;
        }

        // 获取资源路径
        public override String GetPrefabPath()
        {
            return "Social/UViewMasterIssueList";
        }
        ...
        public override void OnMessage(WndMsgID msgID, UIMsgData msgData)
        {
            switch (msgID)
            {
                case WndMsgID.WND_MSG_MENTORSHIP_UPDATEPERSONBASE:
                    {
                        if (m_wndView != null)
                            m_wndView.SetMasterOperateBtn();
                    }
                    break;
                case WndMsgID.WND_MSG_MENTORSHIP_MASTERISSUELIST:
                    {
                        if (m_wndView != null)
                            m_wndView.OnRecvMasterIssueList();
                    }
                    break;
                default:
                    break;
        ...
        public void RequestMasterIssueListInfo()
        {
    ...
    public class MasterIssueListItem : MonoBehaviour, IPointerDownHandler
    {
        public Text name;
        public Text level;
        ...
        private MentorshipMasterListWndView wndView;
        public void SetWndView(MentorshipMasterListWndView view, int index)
        {
            wndView = view;
            m_index = index;
        }
        ...
        public void SetData(cmd_mentorship_issueinfo data)
        {
            ...
            SetIssueItemStateDesc(LogicDataCenter.mentorshipDataManager.IsMApplyFreezing(m_PersonPDBID));
        ...
        public void SetIssueItemStateDesc(bool bState)
        {
            if (bState)
            {
                stateDesc.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "IssueItemStateDesc");
            }
            else
            {
                stateDesc.text = "";
    ...
    public class MentorshipMasterListWndView : UIBaseSubWndView
    {
        public MasterIssueListItem[] listIssueItem;
        ...
        public Button internMasterBtn;
        public Text internMasterBtnText;
        ...
        public override bool Init(IUISubWnd wnd)
        {
            ...
            issueBtnText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "IssueBtnDesc");
            changeIssueBtnText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "ChangeIssueBtnDesc");
            internMasterBtnText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "InternMasterBtnDesc");

            nameText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "NameDesc");
            levelText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "LeveDesc");
            sexText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "SexDesc");
            corpsText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "CorpsNameDesc");
            clanText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "ClanNameDesc");
            issueText.text = ULocalizationService.Instance.Get("UIView", "MasterIssueList", "IssueDesc");
        ...
        public void OnRecvMasterIssueList()
        {
            OnSelectIssue(0, 0, null);
            List<cmd_mentorship_issueinfo> issueInfoList = LogicDataCenter.mentorshipDataManager.MasterIssueList;
            for (int i = 0; i < listIssueItem.Length; ++i)
            {
            ...
            for (int i = 0; i < listIssueItem.Length; ++i)
            {
                if (i < LogicDataCenter.mentorshipDataManager.MASTERISSUECOUNT)
                {
        ...
        {
            UISystem.Instance.ShowWnd(WndID.WND_ID_MASTER_MSGBOXISSUE, true);
        }

        public void OnChangeIssueBtnClick()
        {
            MentorshipMasterListWnd wnd = m_wnd as MentorshipMasterListWnd;
            wnd.RequestMasterIssueListInfo();
        }

        public void OnInternMasterBtnClick()
        {
            if (m_curSelPersonPDBID <= 0)
            {
                UIUtil.ShowSystemMessage(EMChatTipID.CHAT_TIP_MENTORSHIP_TUTOR_NOT_SELECTED);
                return;
            }
            ...
            {
                UIUtil.ShowSystemMessage(EMChatTipID.CHAT_TIP_MENTORSHIP_NOT_APPLY_MASTER);
                return;
            }

            if (LogicDataCenter.mentorshipDataManager.IsMApplyFreezing(m_curSelPersonPDBID))
            {
            ...
            LogicDataCenter.mentorshipDataManager.AddMApplyRecord(m_curSelPersonPDBID, Time.unscaledTime + 60f);
        }

        public void OnSelectIssue(int index, int nPresonPDBID, MasterIssueListItem selItem)
        {
        ...
        public void SetMasterOperateBtn()
        {
            bool bIssueBtnVisible = true;
            bool bMasterBtnVisible = true;
            ...
            htmlString = (bMasterBtnVisible ? "#ffffff" : "#bebebe");
            ColorUtility.TryParseHtmlString(htmlString, out clr);
            internMasterBtn.interactable = bMasterBtnVisible;
            internMasterBtnText.color = clr;
        }
    }
}
