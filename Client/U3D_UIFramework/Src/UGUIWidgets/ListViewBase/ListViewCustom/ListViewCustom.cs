...
** 日  期:	2016-4-14
** 版  本:	1.8.0
** 描  述:	自定义的ListView基类，每种ListView都与对应的ListViewItem相关
		    ListViewCustom<TComponent,TItem> TComponent为ListView的显示对象
                                             TItem为ListView显示对象所需的数据类型
********************************************************************/
...
namespace UIWidgets
{
	...
	/// </summary>
	public class ListViewCustom<TComponent,TItem> : ListViewBase where TComponent : ListViewItem where TItem : new()
    {
        ...
        /// <para>WillNote:当前Rect可以显示的最大Item数量</para>
		/// </summary>
		protected int maxVisibleItems;

		/// <summary>
		/// Count of visible items.
        /// <para>WillNote:当前Rect显示的Item数量</para>
		/// </summary>
		...
		{
            TItem DataItem = DataSource[index];

            if (SelectedItemsCache!=null && !SelectedItemsCache.Contains(DataItem))
			{
				SelectedItemsCache.Add(DataItem);
			}
		...
		void AddComponent(int index)
		{
			...
			else
			{
                ...
                component.transform.SetParent(Unused.transform, false);
		...
		protected virtual void ScrollUpdate()
		{
			...
			else
			{
                ...
                if(new_indicies_to_change.Count != components_to_change.Count)
                {
                    Debug.LogWarning(String.Format("new_indicies_to_change.Count != components_to_change.Count,count1={0},count2{1}",
                        new_indicies_to_change.Count, components_to_change.Count));
                }

				new_indicies_to_change.ForEach(index => {

                    if (components_to_change.Count > 0)
                    {
        ...
        /// <para>WillNote:滚动时更新显示对象显示的数据</para>
        /// </summary>
		...
		protected virtual void SetNewItems(ObservableList<TItem> newItems)
		{
            ...
            if (UserItemCount > 0)
            {
                int delta = UserItemCount - newItems.Count;

                for (int i = 0; i < delta; ++i)
                {
                    newItems.BeginUpdate();
                    TItem m_nUserEmptyItem = new TItem();
                    newItems.Add(m_nUserEmptyItem);
                    newItems.EndUpdate();
                }
            }
            ...
            //WillNote:取消旧的选择，然后根据之前选择的序号在新数据中中心选择已选序号
		...
		void MarkLayoutForRebuild(ListViewItem item)
		{
            if(item == null)
            {
                Debug.LogWarning("ListViewCustom, MarkLayoutForRebuild, item == null");
                return;
            }
