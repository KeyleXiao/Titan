/*******************************************************************
** 文件名:	ListViewCustomHeight.cs 
** 版  权:	NewUIWidgets
** 创建人:	NewUIWidgets
** 日  期:	2016-4-14
** 版  本:	1.8.0
** 描  述:	扩展ListViewCustom类，每个Item的高度是可变的
...
namespace UIWidgets
{
		...
		where TItem: IItemHeight,new()
	{
		...
		protected bool ForceAutoHeightCalculation = true;
		...
		protected float GetItemsHeight(int start, int count)
		{
			if (count==0)
			{
				return 0f;
        ...
        protected float GetItemHeight(TItem item)
		{
			return item.Height;
		}
        ...
        /// <para>WillNote:这里基本上属于这个类的重点了，根据每个Item的高度算显示Item的数量</para>
		/// </summary>
		...
		protected override void AddCallback(ListViewItem item, int index)
		{
            if (item == null)
            {
                Debug.LogWarning("ListViewCustomHeight, AddCallback, item == null");
                return;
            }
		...
		protected override void RemoveCallback(ListViewItem item, int index)
		{
            if (item == null)
            {
                Debug.LogWarning("ListViewCustomHeight, RemoveCallback, item == null");
                return;
            }
		...
		#if UNITY_EDITOR
		protected bool IsItemCanCalculateHeight()
		{
