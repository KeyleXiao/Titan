/*******************************************************************
** 文件名:	TreeViewCustom.cs 
** 版  权:	NewUIWidgets
** 创建人:	NewUIWidgets
** 日  期:	2016-4-16
** 版  本:	1.8.0
** 描  述:	自定义树形列表基类，主要是把树节点数据（TreeNode）信息转为列表节点（NodeList）
            次要是展开、收缩树节点的操作。
...
namespace UIWidgets {
	...
	/// TreeViewCustom.
	/// </summary>
	public class TreeViewCustom<TComponent,TItem>
        : ListViewCustom<TComponent,ListNode<TItem>>
        where TComponent : ListViewItem
	{
		...
		[SerializeField]
		protected IObservableList<TreeNode<TItem>> nodes = new ObservableList<TreeNode<TItem>>();
        ...
        protected void SetParent(TreeNode<TItem> node)
		{
        ...
        /// <para>WillNote:把数据TreeNote根据层级关系转换为列表节点ListNode</para>
		/// </summary>
