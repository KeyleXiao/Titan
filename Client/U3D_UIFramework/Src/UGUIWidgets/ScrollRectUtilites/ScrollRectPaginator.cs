...
namespace UIWidgets {
	...
	public class ScrollRectPaginator : MonoBehaviour {
		...
		protected virtual float GetPageSize()
		{
			...
			if (IsHorizontal())
			{
				return (ScrollRect.transform as RectTransform).rect.width;
			}
			else
			{
				return (ScrollRect.transform as RectTransform).rect.height;
			}
		}
		...
		/// Go to previous page.
		/// </summary>
		public virtual void Prev(int x)
		{
			Prev();
		}
