...
void _pthread_once_cleanup(pthread_once_t *o)
{
	*o = 0;
}
...
int pthread_once(pthread_once_t *o, void(*func)(void))
{
	...
	while (state != 1)
	{
		if (!state)
		{
			...
			{
				/* Success */
				pthread_cleanup_push((_pthread_cleanup::Func)_pthread_once_cleanup, o);
				func();
...
int pthread_key_create(pthread_key_t *key, void(*dest)(void *))
{
	...
	d = (func)realloc(_pthread_key_dest, nmax * sizeof(*d));
	if (!d)
	{
		pthread_rwlock_unlock(&_pthread_key_lock);
...
int pthread_cond_wait(pthread_cond_t *c, pthread_rwlock_t *m)
{
	pthread_testcancel();
