...
void CRedisCommand::SetArgs(const std::vector<std::string> &vecArg)
{
	...
	for (auto strArg = vecArg.begin(); strArg != vecArg.end(); ++strArg)
		AppendValue(*strArg);
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::vector<std::string> &vecArg2)
{
    ...
    AppendValue(strArg1);
	for (auto strArg = vecArg2.begin(); strArg != vecArg2.end(); ++strArg)
		AppendValue(*strArg);
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::set<std::string> &setArg2)
{
    ...
    AppendValue(strArg1);
	for (auto strArg = setArg2.begin(); strArg != setArg2.end(); ++strArg)
		AppendValue(*strArg);
...
void CRedisCommand::SetArgs(const std::vector<std::string> &vecArg1, const std::string &strArg2)
{
	...
	for (auto strArg = vecArg1.begin(); strArg != vecArg1.end(); ++strArg)
		AppendValue(*strArg);
...
void CRedisCommand::SetArgs(const std::map<std::string, std::string> &mapArg)
{
	...
	for (auto kvPair = mapArg.begin(); kvPair != mapArg.end(); ++kvPair)
	{
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::map<std::string, std::string> &mapArg2)
{
    ...
    AppendValue(strArg1);
	for (auto kvPair = mapArg2.begin(); kvPair != mapArg2.end(); ++kvPair)
	{
		AppendValue(kvPair->first);
		AppendValue(kvPair->second);
	}
...
void CRedisCommand::SetArgs(const std::string &strArg1, const std::string &strArg2, const std::vector<std::string> &vecArg3)
{
    ...
    AppendValue(strArg1);
    AppendValue(strArg2);
	for (auto strArg = vecArg3.begin(); strArg != vecArg3.end(); ++strArg)
		AppendValue(*strArg);
...
bool CRedisConnection::Reconnect()
{
	...
	for (auto hostPair = m_pRedisServ->m_vecHosts.begin(); hostPair != m_pRedisServ->m_vecHosts.end(); ++hostPair)
    {
...
{
	for (auto pRedisCmd = m_vecCmd.begin(); pRedisCmd != m_vecCmd.end(); ++pRedisCmd)
		delete *pRedisCmd;
...
void CRedisPipeline::QueueCommand(CRedisCommand *pRedisCmd, TFuncFetch& funcFetch)
{
    m_vecCmd.push_back(pRedisCmd);
	m_vecFuncFetch.push_back(std::move(funcFetch));
}

void CRedisPipeline::FetchResults()
{
	size_t size = m_vecCmd.size();
	for (int i=0; i<size; ++i)
	{
		auto cmd = m_vecCmd.at(i);
		TFuncFetch& func = m_vecFuncFetch.at(i);
		cmd->FetchResult(func);
	}
}
      ...
      m_bValid(true), m_bExit(false), m_pThread(nullptr)
{
	...
	pthread_cond_init(&m_condAny, nullptr);
}
...
CRedisClient::~CRedisClient()
{
    ...
    m_bExit = true;
    {
		...
		pthread_cond_broadcast(&m_condAny);
    }
    if (m_pThread)
    {
        //m_pThread->join();
	...
	pthread_cond_destroy(&m_condAny);
}
...
bool CRedisClient::Initialize(const std::string &strHost, int nPort, int nTimeout, int nConnNum)
{
	...
	m_bValid = (m_bCluster ? LoadClusterSlots() : LoadSlaveInfo(mapInfo)) &&
	    (m_pThread = new rkt::Thread()) != nullptr && m_pThread->spawn(this);
    return m_bValid;
}
    ...
    {
        {
            ...
            if (m_bValid)
				...
				pthread_cond_wait(&m_condAny, &m_rwLock);
...
{
	return ExecuteImpl("del", strKey, HASH_SLOT(strKey), ppLine, BIND_INT(pnVal), FUNC_DEF_CONV);
}
...
int CRedisClient::Keys(const std::string &strPattern, std::vector<std::string> *pvecVal)
{
    ...
    for (auto pRedisServ = m_vecRedisServ.begin(); pRedisServ != m_vecRedisServ.end(); ++pRedisServ)
	{
...
int CRedisClient::Pexpireat(const std::string &strKey, long long nMilliTime, long *pnVal, Pipeline ppLine)
{
...
int CRedisClient::Mget(const std::vector<std::string> &vecKey, std::vector<std::string> *pvecVal)
{
    ...
    else
    {
        ...
        for	(auto strKey = vecKey.begin(); strKey != vecKey.end(); ++strKey)
		{
...
int CRedisClient::Zrangebyscore(const std::string &strKey, double dMin, double dMax, std::map<std::string, double> *pmapVal, Pipeline ppLine)
{
    ...
    if (nRet == RC_SUCCESS && pmapVal)
    {
		...
		for (auto memPair = mapVal.begin(); memPair != mapVal.end(); ++memPair)
			pmapVal->insert(std::make_pair(memPair->first, atof(memPair->second.c_str())));
    }
    return nRet;
}
...
int CRedisClient::FlushAll()
{
	return ExecuteImpl("flushall", -1, nullptr, BIND_STR(nullptr));
}
int CRedisClient::FlushDb()
{
	return ExecuteImpl("flushdb", -1, nullptr, BIND_STR(nullptr));
}

int CRedisClient::ExecuteImpl(const std::string &strCmd, int nSlot, Pipeline ppLine, TFuncFetch funcFetch)
{
	CRedisCommand *pRedisCmd = new CRedisCommand(strCmd, !ppLine);
	pRedisCmd->SetArgs();
	pRedisCmd->SetSlot(nSlot);
	pRedisCmd->SetConvFunc(FUNC_DEF_CONV);
	int nRet = Execute(pRedisCmd, funcFetch, ppLine);
...
int CRedisClient::FetchReplys(Pipeline ppLine)
{
	if (!ppLine)
		return RC_PIPELINE_ERR;

	CRedisPipeline *pPipeline = static_cast<CRedisPipeline *>(ppLine);
	pPipeline->FetchResults();
...
bool CRedisClient::LoadClusterSlots()
{
    ...
    for (size_t i = 0; i < m_vecRedisServ.size(); ++i)
    {
        ...
        if (!pRedisServ->IsValid())
        {
            ...
            //    delete pRedisServ;
			for (auto pRedisServ = vecRedisServ.begin(); pRedisServ != vecRedisServ.end(); ++pRedisServ)
				delete *pRedisServ;
            return false;
        }
            ...
            redisCmd.FetchResult(BIND_SLOT(&vecSlot)) == RC_SUCCESS)
        {
			...
			for (auto slotReg = vecSlot.begin(); slotReg != vecSlot.end(); ++slotReg)
            {
...
int CRedisClient::Execute(CRedisCommand *pRedisCmd, TFuncFetch& funcFetch, Pipeline ppLine)
{
    if (ppLine)
    {
        CRedisPipeline *pPipeline = static_cast<CRedisPipeline *>(ppLine);
        pPipeline->QueueCommand(pRedisCmd, funcFetch);
        return RC_SUCCESS;
    }
