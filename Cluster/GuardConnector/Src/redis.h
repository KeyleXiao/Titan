...
private:
    ...
    void QueueCommand(CRedisCommand *pRedisCmd, TFuncFetch& funcFetch);
...
private:
	...
	std::vector<TFuncFetch> m_vecFuncFetch;
    size_t m_nCursor;
};

typedef void * Pipeline;
class CRedisClient : public rkt::IRunnable
{
...
public:
	...
	int FetchReplys(Pipeline ppLine);
    ...
    int Pexpireat(const std::string &strKey, long long nMilliTime, long *pnVal = nullptr, Pipeline ppLine = nullptr);
...
private:
    ...
    bool WaitForRefresh();
    int Execute(CRedisCommand *pRedisCmd, TFuncFetch& funcFetch, Pipeline ppLine = nullptr);
...
private:
    ...
    pthread_cond_t m_condAny;
    rkt::Thread *m_pThread;
};
