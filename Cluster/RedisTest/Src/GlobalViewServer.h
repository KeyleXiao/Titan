...
class GlobalViewServer : public IGlobalViewServer
						 , public TimerHandler
						 , public EventHandler
...
public:
	...
	@name                    : 初始化网络
	@brief                   :
	@param dwServerID		 : 本ServerID
	@param szMngIP			 : 管理服IP
	@param wMngPort			 : 管理服端口
	@return                  :
	*/
	bool Create( DWORD dwServerID, const char * szMngIP, WORD wMngPort);
...
public:
	...
	Processer<MngConnector>		m_processer;		// 消息分发器
	ByteRecord					m_brMng;			// Mng连接收发记录器
	IManagerConnector			*m_ManagerConnector;// 服务器管理连接器

	MngConnector				m_MngConnector;		// 管理服连接
...
private:
	...
	bool	m_bInit;	// 是否初始化
	bool	m_bStart;	// 是否启动
	bool	m_bReady;	// 网关是否与观战管理服已经握手完毕
};
