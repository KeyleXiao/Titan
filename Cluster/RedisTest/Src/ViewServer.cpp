...
BEGIN_MESSAGE_MAP(CViewServerApp, CWinApp)
...
bool IsProcessExist(DWORD dwServerID)
{
	// 唯一标识字串，用来创建互斥
	CString strTemp;
	strTemp.Format( _T("观战Server[%d] 已经在运行!"), dwServerID );
	//干脆就用这个提示文本做互斥key
...
// CViewServerApp 初始化
BOOL CViewServerApp::InitInstance()
{
	...
	// 用new的方式以免对象太大堆栈不够用
	CViewServerDlg *pDlg = new CViewServerDlg;
	gSetting.SetServerID(dwServerID);
	m_pMainWnd = pDlg;
