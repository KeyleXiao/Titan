...
CViewMngDlg::CViewMngDlg(CWnd* pParent /*=NULL*/)
	: CResizingDialog(CViewMngDlg::IDD, pParent),m_trayIcon(IDR_TRAYICON)
	, m_strWorld(_T(""))
	, m_strIsTemp(_T(""))
	, m_strGame(_T(""))
	, m_strArea(_T(""))
...
void CViewMngDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CViewMngDlg, CResizingDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_WM_CLOSE()
	ON_COMMAND(ID_APP_OPEN, &CViewMngDlg::OnAppOpen)
	ON_COMMAND(ID_APP_ABOUT, &CViewMngDlg::OnAppAbout)
	ON_COMMAND(ID_APP_SUSPEND, &CViewMngDlg::OnAppSuspend)
	ON_BN_CLICKED(IDC_BUTTON_START, &CViewMngDlg::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CViewMngDlg::OnBnClickedButtonStop)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_EXIT, &CViewMngDlg::OnBnClickedButtonExit)
	ON_COMMAND(IDC_START, &CViewMngDlg::OnStart)
	ON_COMMAND(IDC_STOP, &CViewMngDlg::OnStop)
	ON_COMMAND(ID_OUT_TOTAL, &CViewMngDlg::OnOutTotal)
	ON_COMMAND(ID_SHOW_LISTINFO, &CViewMngDlg::OnShowListinfo)

	ON_MESSAGE(WM_MY_START_RESULT,&CViewMngDlg::OnMsgStartResult)
	ON_MESSAGE(WM_MY_STOP_RESULT,&CViewMngDlg::OnMsgStopResult)
	ON_MESSAGE(WM_MY_CLOSE_APP,&CViewMngDlg::OnMsgCloseApp)
	ON_COMMAND(ID_MG_DLG, &CViewMngDlg::OnMgDlg)
	ON_COMMAND(ID_TRACELEVEL, &CViewMngDlg::OnTracelevel)
END_MESSAGE_MAP()


// CViewMngDlg 消息处理程序

BOOL CViewMngDlg::OnInitDialog()
{
...
void CViewMngDlg::UpdateCenterServerConfig()
{
	UpdateData(TRUE);
}

//更新界面中心服务器ＩＰ,端口
void CViewMngDlg::ShowCenterServerConfig()
{
	UpdateData(FALSE);
}


void CViewMngDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//
HCURSOR CViewMngDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

// 关闭程序
void CViewMngDlg::OnClose()
{
...
// 起动结果message 
LRESULT CViewMngDlg::OnMsgStartResult(WPARAM wParam, LPARAM lParam)
{
...
// 关闭结果message 
LRESULT CViewMngDlg::OnMsgStopResult(WPARAM wParam, LPARAM lParam)
{
	m_bStart =(wParam==1?TRUE:FALSE);
...
// 关闭程序message 
LRESULT CViewMngDlg::OnMsgCloseApp(WPARAM wParam, LPARAM lParam)
{
	// 	退出程处理代码
	CResizingDialog::OnCancel();
...
// 更新日志到输出框
void CViewMngDlg::UpdateTraceInfo2RichEdit()
{
	if (mTrace)
...
//启动服务
void CViewMngDlg::OnBnClickedButtonStart()
{
	// TODO: 在此添加控件通知处理程序代码
...
//停止服务
void CViewMngDlg::OnBnClickedButtonStop()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CViewMngDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
// 退出
void CViewMngDlg::OnBnClickedButtonExit()
{
	// TODO: 在此添加控件通知处理程序代码
	OnAppSuspend();
}
...
void CViewMngDlg::SetTrayIcon(BOOL bIsRuning)
{
	if (bIsRuning)
	{
		//服务在运行,设置tray icon
...
void CViewMngDlg::OnStart()
{
	// TODO: 在此添加命令处理程序代码
	OnBnClickedButtonStart();
}

void CViewMngDlg::UpdateMenuState()
{
	CString pstr;
...
// 刷新统计信息
void CViewMngDlg::UpdateTotalInfo()
{
	// 服务务器更新和相关处理
	m_VoiceServer.m_TotalInfo.dwServers	= gGameList.Count();		// 在线服务器数
	m_VoiceServer.m_TotalInfo.dwClients = gViewList.Count();	// ViewServer数 TODO
...
void CViewMngDlg::OnShowListinfo()
{
	// TODO: 在此添加命令处理程序代码
...
void CViewMngDlg::OnMgDlg()
{
	...
	if( opDlg.m_nNumber1 < VoiceCommand_ChangeMaxConnection || opDlg.m_nNumber1 >= VoiceCommand_Max )
	{
