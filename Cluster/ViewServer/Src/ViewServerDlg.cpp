...
#define EXIT_CONFIRM_TEXT _T("观战Server正在运行中，你确定要停止服务并退出程序吗？")
...
CViewServerDlg::CViewServerDlg(CWnd* pParent /*=NULL*/)
	: CResizingDialog(CViewServerDlg::IDD, pParent),m_trayIcon(IDR_TRAYICON)
	, m_strWorld(_T(""))
	, m_strIsTemp(_T(""))
	, m_strGame(_T(""))
	, m_strArea(_T(""))
...
void CViewServerDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CViewServerDlg, CResizingDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_WM_CLOSE()
	ON_COMMAND(ID_APP_OPEN, &CViewServerDlg::OnAppOpen)
	ON_COMMAND(ID_APP_ABOUT, &CViewServerDlg::OnAppAbout)
	ON_COMMAND(ID_APP_SUSPEND, &CViewServerDlg::OnAppSuspend)
	ON_BN_CLICKED(IDC_BUTTON_START, &CViewServerDlg::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CViewServerDlg::OnBnClickedButtonStop)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_EXIT, &CViewServerDlg::OnBnClickedButtonExit)
	ON_COMMAND(IDC_START, &CViewServerDlg::OnStart)
	ON_COMMAND(IDC_STOP, &CViewServerDlg::OnStop)
	ON_COMMAND(ID_OUT_TOTAL, &CViewServerDlg::OnOutTotal)
	ON_COMMAND(ID_SHOW_LISTINFO, &CViewServerDlg::OnShowListinfo)

	ON_MESSAGE(WM_MY_START_RESULT,&CViewServerDlg::OnMsgStartResult)
	ON_MESSAGE(WM_MY_STOP_RESULT,&CViewServerDlg::OnMsgStopResult)
	ON_MESSAGE(WM_MY_CLOSE_APP,&CViewServerDlg::OnMsgCloseApp)
	ON_COMMAND(ID_MG_DLG, &CViewServerDlg::OnMgDlg)
	ON_COMMAND(ID_TRACELEVEL, &CViewServerDlg::OnTracelevel)
END_MESSAGE_MAP()


// CViewServerDlg 消息处理程序

BOOL CViewServerDlg::OnInitDialog()
{
...
void CViewServerDlg::UpdateCenterServerConfig()
{
	UpdateData(TRUE);
}

//更新界面中心服务器ＩＰ,端口
void CViewServerDlg::ShowCenterServerConfig()
{
	UpdateData(FALSE);
}


void CViewServerDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//
HCURSOR CViewServerDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

// 关闭程序
void CViewServerDlg::OnClose()
{
...
// 起动结果message 
LRESULT CViewServerDlg::OnMsgStartResult(WPARAM wParam, LPARAM lParam)
{
...
// 关闭结果message 
LRESULT CViewServerDlg::OnMsgStopResult(WPARAM wParam, LPARAM lParam)
{
	m_bStart =(wParam==1?TRUE:FALSE);
...
// 关闭程序message 
LRESULT CViewServerDlg::OnMsgCloseApp(WPARAM wParam, LPARAM lParam)
{
	// 	退出程处理代码
	CResizingDialog::OnCancel();
...
void CViewServerDlg::OnAppAbout()
{
	// TODO: 在此添加命令处理程序代码
	CAboutDlg dlgAbout;
	dlgAbout.DoModal();
...
// 点任务栏图标菜单的"退出"关闭程序
void CViewServerDlg::OnAppSuspend()
{
	// TODO: 在此添加命令处理程序代码
...
// 更新日志到输出框
void CViewServerDlg::UpdateTraceInfo2RichEdit()
{
	if (mTrace)
...
//启动服务
void CViewServerDlg::OnBnClickedButtonStart()
{
	// TODO: 在此添加控件通知处理程序代码
...
//停止服务
void CViewServerDlg::OnBnClickedButtonStop()
{
	// TODO: 在此添加控件通知处理程序代码
...
//停止服务
bool CViewServerDlg::StopService()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CViewServerDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
// 退出
void CViewServerDlg::OnBnClickedButtonExit()
{
	// TODO: 在此添加控件通知处理程序代码
	OnAppSuspend();
}
...
void CViewServerDlg::SetTrayIcon(BOOL bIsRuning)
{
	if (bIsRuning)
	{
		//服务在运行,设置tray icon
...
void CViewServerDlg::OnStart()
{
	// TODO: 在此添加命令处理程序代码
	OnBnClickedButtonStart();
}

void CViewServerDlg::UpdateMenuState()
{
	CString pstr;
...
// 刷新统计信息
void CViewServerDlg::UpdateTotalInfo()
{
	...
	m_strServerPort.Format(_T(" %s:%d"), a2t(gSetting.m_strMngIp.c_str()), gSetting.m_dwMngPort);								// 服务器IP
...
void CViewServerDlg::OnOutTotal()
{
	// TODO: 在此添加命令处理程序代码
	char szBuf[512] = {0};
...
void CViewServerDlg::OnShowListinfo()
{
	// TODO: 在此添加命令处理程序代码
...
void CViewServerDlg::OnTracelevel()
{
	// TODO: 在此添加命令处理程序代码
