...
#include "VoiceCommonPart.h"

// 全局管理连接器接口
VoiceConnector* g_hVoiceConnector = NULL;


//////////////////////////////////////////////////////////////////////////////
API_EXPORT  IVoiceConnector * RKT_CDECL CreateVoiceConnector(rkt::TimerAxis * pTimerAxis)
{
	VoiceConnector * pConn = new VoiceConnector(pTimerAxis);
	return (IVoiceConnector *)pConn;
}

//////////////////////////////////////////////////////////////////////////
VoiceConnector::VoiceConnector(TimerAxis * pTimerAxis):
	 m_pConnection(0)
    ...
    , m_nThreadMode(NetworkUserType_Default)
{
	...
	if (NULL!=g_hVoiceConnector)
	{
		ErrorLn(_GT("语音服务器连接器被重复创建，会导致服务器管理功能出问题，请检查相关代码并修正！"));
	}
	g_hVoiceConnector = this;
}

VoiceConnector::~VoiceConnector()
{
	g_hVoiceConnector = NULL;
}

bool VoiceConnector::Connect(const char * szCSIP,WORD nCSPort,int nThreadMode)
{
...
bool VoiceConnector::Create(DWORD dwGameWorldID,WORD wServerType,WORD wSubID,const char * szServerName,const char * szWorldName)
{
	// 保存参数
...
bool VoiceConnector::RegisterServer(DWORD dwGameWorldID,WORD wServerType,WORD wSubID)
{
	m_dwWorldID = dwGameWorldID;
	m_wSubID = wSubID;
	m_wServerType = wServerType;
...
DWORD VoiceConnector::GetServerID()
{
	return m_ID;
}
...
*/
bool VoiceConnector::registerEventHandler(WORD wEventID,IVoiceEventHandler* handler)
{
...
@param  hander 事件处理者实例
*/
bool VoiceConnector::unregisterEventHandler(WORD wEventID,IVoiceEventHandler* handler)
{
	if (NULL==handler|| wEventID>=EVENT_VOICE_MAXID)
	{
...
@param wEventID 事件ID	参考 VoiceDef.h   EMEventVoice
*/
bool VoiceConnector::FireEventExecute(WORD wEventID, LPSTR pszContext, size_t nLen)
{
...
*/
bool VoiceConnector::registerMessageHandler(DWORD moduleId, IVoiceMessageHandler* handler)
{
...
@param moduleId 模块ID
*/
bool VoiceConnector::unregisterMessageHandler(DWORD moduleId)
{
	Assert(moduleId >= 0 && moduleId < MSG_MODULEID_MAXID);
	if (moduleId >= 0 && moduleId < MSG_MODULEID_MAXID)
	{
...
// 语音服务器连接器接口
IVoiceConnector* GetGlobalVoiceConnector()
{
	return g_hVoiceConnector;
}
...
void VoiceConnector::SendDataToServer(DWORD dwServerID,const char * pData,DWORD dwLen,bool bZeroBroadcast)
{
...
void VoiceConnector::BroadcastDataToServer(DWORD * pServerArray,WORD wServerNum,const char * pData,DWORD dwLen)
{
...
@return  找到的个数
*/
DWORD VoiceConnector::GetGameServerIDList(DWORD * pServerArray,DWORD dwArrayLen,DWORD &dwGetCounts,DWORD dwWorldID,WORD wServerType)
{
...
*/
DWORD VoiceConnector::GetGameServerIDByTypeSubID(DWORD dwWorldID,WORD wServerType,WORD wSubID)
{
	// 处理 游戏世界ID分类列表
	TMAP_VoiceWorldIDInfo::iterator iter = m_mapWorldList.find(dwWorldID);
	if ( iter==m_mapWorldList.end())
...
@return 
*/
void VoiceConnector::PrintGameServerListInfo(DWORD dwWorldID,WORD wServerType)
{
	for(TMAP_VoicePartList::iterator iter = m_mapPartList.begin(); iter!= m_mapPartList.end();++iter)
	{
		if ( (0==dwWorldID|| dwWorldID==iter->second->GetWorldID()) && (0==wServerType|| wServerType==iter->second->GetServerType()))
...
@return  找到的个数
*/
DWORD VoiceConnector::GetGameWorldIDList(DWORD * pWorldArray,DWORD dwArrayLen,DWORD &dwGetCounts)
{
...
@return  找到的服务器ID,为0表示没找到
*/
IVoiceWorldInfo * VoiceConnector::GetGameWorldInfo(DWORD dwWorldID)
{
	// 游戏世界ID分类列表
	TMAP_VoiceWorldIDInfo::iterator iter = m_mapWorldList.find(dwWorldID);
	if ( iter==m_mapWorldList.end())
...
@return 
*/
void VoiceConnector::PrintGameWorldListInfo()
{
	// 游戏世界ID分类列表,以dwWorldID为关键字
	for(TMAP_VoiceWorldIDInfo::iterator iter = m_mapWorldList.begin(); iter!= m_mapWorldList.end();++iter)
	{
...
*/
IVoicePart* VoiceConnector::GetGameServerPart(DWORD dwServerID)
{
...
*/
IVoicePart* VoiceConnector::GetGameServerPartByTypeSubID(DWORD dwWorldID,WORD wServerType,WORD wSubID)
{
...
// 清除所有服务器信息列表
void VoiceConnector::ClearAllList(void)
{
	// 语音服务器上的服务器扩展信息类列表,以服务器ID为关键字
	for(TMAP_VoicePartList::iterator iter = m_mapPartList.begin(); iter!= m_mapPartList.end();++iter)
	{
...
*/
bool VoiceConnector::AddVoiceBlueDiamond( DWORD dwUDBID, DWORD dwCount, DWORD dwPDBID, DWORD dwPDBIDSend, DWORD dwSpare1, DWORD dwSpare2, DWORD dwSpare3, const char * szReason )
{
...
*/
bool VoiceConnector::BroadcastDataToClient( BYTE nBroadcastType, void * pData, DWORD dwLen, DWORD dwPDBID1, DWORD dwPDBID2, DWORD dwWorldID1, DWORD dwWorldID2, DWORD dwValueID1, DWORD dwValueID2, bool bRoom )
{
...
void VoiceConnector::HandleVoiceMessage(ulong actionId,SMsgVoiceHead &voiceHead,SGameMsgHead* head, LPSTR data, size_t len)
{
	switch(actionId)
	{
...
// 回应握手消息
void VoiceConnector::OnMsgHandshakeResponse(unsigned long actionId, SMsgVoiceHead &voiceHead,SGameMsgHead* head, LPSTR data, size_t len)
{
...
// 服务器列表更新消息
void VoiceConnector::OnMsgServerListUpdate(unsigned long actionId, SMsgVoiceHead &voiceHead, SGameMsgHead* head, LPSTR data, size_t len)
{
	...
	// 调式代码
	char szBuf[512]={0};
	sprintf_s(szBuf, _countof(szBuf),_NGT"VoiceConnector::OnMsgServerListUpdate()  服务器列表更新消息:来源游戏世界ID=%d,来源服务器ID=%d,更新类型=%d,软件服务器个数=%d",
...
// 游戏世界列表更新消息
void VoiceConnector::OnMsgWorldListUpdate(unsigned long actionId, SMsgVoiceHead &voiceHead, SGameMsgHead* head, LPSTR data, size_t len)
{
	...
	// 调式代码
	char szBuf[512]={0};
	sprintf_s(szBuf, _countof(szBuf),_NGT"VoiceConnector::OnMsgWorldListUpdate()  游戏世界列表更新消息:来源游戏世界ID=%d,来源服务器ID=%d,更新类型=%d,游戏世界个数=%d",
...
@return  
*/
TimerAxis* VoiceConnector::GetTimerAxis()
{
	return m_pTimerAxis;
}
