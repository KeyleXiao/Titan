...
void COperationDlg::OnOK()
{
	...
	{
		//sprintf_s(szBuf,sizeof(szBuf), _T("请输入[%s],内容最少%d字符！"),m_strTxtName.GetString(),m_nMinTxtLens);
        CString strInfo;
        strInfo.Format( _T("请输入[%s],内容最少%d字符！"),m_strTxtName.GetString(),m_nMinTxtLens);
		MessageBox(strInfo,_T("提示"),MB_ICONINFORMATION);
		GetDlgItem(IDC_EDIT_TXT)->SetFocus();
		return;
	}
	else if (m_strTxt.GetLength()>=m_nMaxTxtLens)
	{
		//sprintf_s(szBuf,sizeof(szBuf), _T("请输入[%s],内容最多%d字符！"),m_strTxtName.GetString(),m_nMaxTxtLens);
        CString strInfo;
        strInfo.Format( _T("请输入[%s],内容最多%d字符！"),m_strTxtName.GetString(),m_nMaxTxtLens);
		MessageBox(strInfo,_T("提示"),MB_ICONINFORMATION);
		GetDlgItem(IDC_EDIT_TXT)->SetFocus();
		return;
	}
	...
	{
		TCHAR szVerifyCode[128] = {0};
		m_editDelays.GetWindowText(szVerifyCode,sizeof(szVerifyCode));
		CString strVerifyCode = szVerifyCode;

        CString strTestCode;
        strTestCode.Format(_T("%d"),m_dwVerifyCode);
		//TraceLn("输入:"<<strVerifyCode.data()<<",验证码："<<strTestCode);
		if (strVerifyCode!=strTestCode)
		{
			MessageBox(_T("输入的验证码不正确！"), _T("提示"), MB_ICONINFORMATION);
...
BOOL COperationDlg::OnInitDialog()
{
	...
	if (m_bEnterVerifyCode)
	{
		...
		m_staticDelays.SetWindowText( a2w(szBuf));
	}
