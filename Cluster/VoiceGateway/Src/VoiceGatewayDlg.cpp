/*******************************************************************
** 文件名:	E:\Rocket\Cluster\VoiceGateway\Src\VoiceGatewayDlg.cpp
** 版  权:	(C) 深圳冰川网络技术有限公司
...
CVoiceGatewayDlg::CVoiceGatewayDlg(CWnd* pParent /*=NULL*/)
	: CResizingDialog(CVoiceGatewayDlg::IDD, pParent),m_trayIcon(IDR_TRAYICON)
	, m_strWorld(_T(""))
	, m_strIsTemp(_T(""))
	, m_strGame(_T(""))
	, m_strArea(_T(""))
...
void CVoiceGatewayDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CVoiceGatewayDlg, CResizingDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_WM_CLOSE()
	ON_COMMAND(ID_APP_OPEN, &CVoiceGatewayDlg::OnAppOpen)
	ON_COMMAND(ID_APP_ABOUT, &CVoiceGatewayDlg::OnAppAbout)
	ON_COMMAND(ID_APP_SUSPEND, &CVoiceGatewayDlg::OnAppSuspend)
	ON_BN_CLICKED(IDC_BUTTON_START, &CVoiceGatewayDlg::OnBnClickedButtonStart)
	ON_BN_CLICKED(IDC_BUTTON_STOP, &CVoiceGatewayDlg::OnBnClickedButtonStop)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_EXIT, &CVoiceGatewayDlg::OnBnClickedButtonExit)
	ON_COMMAND(IDC_START, &CVoiceGatewayDlg::OnStart)
	ON_COMMAND(IDC_STOP, &CVoiceGatewayDlg::OnStop)
	ON_COMMAND(ID_OUT_TOTAL, &CVoiceGatewayDlg::OnOutTotal)
	ON_COMMAND(ID_SHOW_LISTINFO, &CVoiceGatewayDlg::OnShowListinfo)

	ON_MESSAGE(WM_MY_START_RESULT,&CVoiceGatewayDlg::OnMsgStartResult)
	ON_MESSAGE(WM_MY_STOP_RESULT,&CVoiceGatewayDlg::OnMsgStopResult)
	ON_MESSAGE(WM_MY_CLOSE_APP,&CVoiceGatewayDlg::OnMsgCloseApp)
	ON_COMMAND(ID_MG_DLG, &CVoiceGatewayDlg::OnMgDlg)
	ON_COMMAND(ID_TRACELEVEL, &CVoiceGatewayDlg::OnTracelevel)
END_MESSAGE_MAP()


// CVoiceGatewayDlg 消息处理程序

BOOL CVoiceGatewayDlg::OnInitDialog()
{
	...
	// 读取本服务器ID，和端口配置
	VoiceGatewaySetting::getInstance().SetServerID(m_dwServerID);
	...
	sprintf_s(szLogAbsolutePath, "%s\\Log\\VoiceGateway%d",m_VoiceGateway.m_szWorkDir,m_dwServerID);
	...
	sprintf_s(m_VoiceGateway.szTraceFileName, "%s\\Log\\VoiceGateway%d\\VoiceGT%d%.2d%.2d%.2d%.2d%.2d_Trace.html", m_VoiceGateway.m_szWorkDir,m_dwServerID,
		...
		local_time->tm_min,
		local_time->tm_sec);
	sprintf_s(m_VoiceGateway.szCrashFileName, "%s\\Log\\VoiceGateway%d\\VoiceGT%d%.2d%.2d%.2d%.2d%.2d_Crash.txt", m_VoiceGateway.m_szWorkDir,m_dwServerID,
		local_time->tm_year+1900,
		local_time->tm_mon + 1,
		local_time->tm_mday,
		local_time->tm_hour,
		local_time->tm_min,
		local_time->tm_sec);
	sprintf_s(m_VoiceGateway.szDumpFileName, "%s\\Log\\VoiceGateway%d\\VoiceGT%d%.2d%.2d%.2d%.2d%.2d_Stack.dmp",  m_VoiceGateway.m_szWorkDir,m_dwServerID,
		local_time->tm_year+1900,
		local_time->tm_mon + 1,
		local_time->tm_mday,
		local_time->tm_hour,
	...
	CString strText;
	strText.Format( _T("语音网关[%d]"), m_dwServerID );
...
void CVoiceGatewayDlg::UpdateCenterServerConfig()
{
	UpdateData(TRUE);
}

//更新界面中心服务器ＩＰ,端口
void CVoiceGatewayDlg::ShowCenterServerConfig()
{
	UpdateData(FALSE);
}


void CVoiceGatewayDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//
HCURSOR CVoiceGatewayDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

// 关闭程序
void CVoiceGatewayDlg::OnClose()
{
...
// 起动结果message 
LRESULT CVoiceGatewayDlg::OnMsgStartResult(WPARAM wParam, LPARAM lParam)
{
...
// 关闭结果message 
LRESULT CVoiceGatewayDlg::OnMsgStopResult(WPARAM wParam, LPARAM lParam)
{
	m_bStart =(wParam==1?TRUE:FALSE);
...
// 关闭程序message 
LRESULT CVoiceGatewayDlg::OnMsgCloseApp(WPARAM wParam, LPARAM lParam)
{
	// 	退出程处理代码
	CResizingDialog::OnCancel();
...
// 点任务栏图标菜单的"打开"显示程序界面
void CVoiceGatewayDlg::OnAppOpen()
{
	// TODO: 在此添加命令处理程序代码
...
void CVoiceGatewayDlg::OnAppAbout()
{
	// TODO: 在此添加命令处理程序代码
	CAboutDlg dlgAbout;
	dlgAbout.DoModal();
...
// 点任务栏图标菜单的"退出"关闭程序
void CVoiceGatewayDlg::OnAppSuspend()
{
	// TODO: 在此添加命令处理程序代码
	...
	{
		if(MessageBox(_T("语音网关正在运行中，你确定要停止服务并退出程序吗？"), _T("操作确认"),MB_YESNO|MB_ICONQUESTION)!=IDYES) 
		{
			return;
		}
...
void CVoiceGatewayDlg::OnCancel()
{
...
// 更新日志到输出框
void CVoiceGatewayDlg::UpdateTraceInfo2RichEdit()
{
	if (mTrace)
...
//启动服务
void CVoiceGatewayDlg::OnBnClickedButtonStart()
{
	// TODO: 在此添加控件通知处理程序代码
...
//停止服务
void CVoiceGatewayDlg::OnBnClickedButtonStop()
{
	// TODO: 在此添加控件通知处理程序代码
...
//停止服务
bool CVoiceGatewayDlg::StopService()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	CString strAlert=_T("语音网关正在运行中，你确定要停止服务并退出程序吗？");
...
void CVoiceGatewayDlg::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
// 退出
void CVoiceGatewayDlg::OnBnClickedButtonExit()
{
	// TODO: 在此添加控件通知处理程序代码
	OnAppSuspend();
}
...
void CVoiceGatewayDlg::SetTrayIcon(BOOL bIsRuning)
{
	if (bIsRuning)
	{
		//服务在运行,设置tray icon
...
void CVoiceGatewayDlg::OnStart()
{
	// TODO: 在此添加命令处理程序代码
	OnBnClickedButtonStart();
}

void CVoiceGatewayDlg::UpdateMenuState()
{
	CString pstr;
...
// 刷新统计信息
void CVoiceGatewayDlg::UpdateTotalInfo()
{

	// 服务务器更新和相关处理
	//m_VoiceGateway.m_TotalInfo.dwServers	= 0;		// 在线服务器数
	m_VoiceGateway.m_TotalInfo.dwClients = CClientList::getInstance().CountUser();	// 客户端数
	m_VoiceGateway.m_TotalInfo.dwVoiceClients	= CAudioList::getInstance().CountUser();	// 在线客户端数

	DWORD dwMaxUserCounts = m_VoiceGateway.m_dwMaxConnection;	// 最大连接数

	SVoiceTotalInfo pTInfo=m_VoiceGateway.m_TotalInfo;
	...
	m_strServers.Format(_T(" %d 最大:%d"),pTInfo.dwGatewayID,dwMaxUserCounts);			// 网关ID
	m_strClients.Format(_T(" %d 语音:%d"),pTInfo.dwClients,pTInfo.dwVoiceClients);		// 客户端数
	m_strServerPort.Format(_T(" %s:%d"), a2t(VoiceGatewaySetting::getInstance().m_strServerIp.c_str()), VoiceGatewaySetting::getInstance().m_dwServerPort);								// 服务器IP
	m_strClientPort.Format(_T(" %d  语音:%d"),pTInfo.dwClientPort,pTInfo.dwAudioPort);				// 客户端监听端口,语音端监听端口
	m_strRecvSpeedServer.Format(_T(" %s/s"),a2t(GetFileSizeString(pTInfo.dwRecvSpeedServer)));			// 语音收包带宽
	m_strSendSpeedServer.Format(_T(" %s/s"),a2t(GetFileSizeString(pTInfo.dwSendSpeedServer)));			// 语音发包带宽
	m_strRecvSpeedClient.Format(_T(" %s/s"),a2t(GetFileSizeString(pTInfo.dwRecvSpeedClient)));			// 语音收包带宽
	m_strSendSpeedClient.Format(_T(" %s/s"),a2t(GetFileSizeString(pTInfo.dwSendSpeedClient)));			// 语音发包带宽
...
void CVoiceGatewayDlg::OnOutTotal()
{
	// TODO: 在此添加命令处理程序代码
	char szBuf[512] = {0};
...
void CVoiceGatewayDlg::OnShowListinfo()
{
	// TODO: 在此添加命令处理程序代码
...
void CVoiceGatewayDlg::OnTracelevel()
{
	// TODO: 在此添加命令处理程序代码
