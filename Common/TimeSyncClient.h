...
public:
    ...
    DWORD GetPing()
    {
        if ( m_dwServerTick!=0 && m_dwPingLatency==0 )
        {
            return 1;
        }

        return m_dwPingLatency; 
    }
    ...
    virtual bool DoStart(const char * strIP,unsigned short nUDPPort,unsigned short mTCPPort)
    {
        return true;
    }
    ...
    virtual void OnTimer( unsigned long dwTimerID )
    {
        if ( dwTimerID==3 )
        {
            DispatchNetworkEvents(100,NetworkUserType_User3 );
        }
    }

    AbstractSyncClient()
        : m_pTimerAixs(0),m_nUPDPort(0),m_nTCPPort(0),m_pTCPConnection(0),m_dwServerTick(0),m_dwPingLatency(0),m_dwLastLatency(0),m_tStartTime(0)
    {
    }

    virtual ~AbstractSyncClient()
    {
        Stop();
    }
...
protected:
    ...
    void Adjust(long delta )
    {
        ...
        WarningLn("TimeSync Adjust " << delta);
    }
...
////////////// 支持TCP和UDP协议连时间同步服务器 //////////////////////////////////////////////////////
class TcpUdpSyncClient : public AbstractSyncClient, public IUDPHandler,public IConnectionEventHandler
{
public:
    TcpUdpSyncClient()
        : m_pUDPSocket(0)
...
protected:
	bool DoSyncUPD()
	{
		...
		if ( m_pUDPSocket==0 )
		{
			m_pUDPSocket = rkt::CreateUDPSocket( 0,this,NetworkUserType_User3 );
			if ( m_pUDPSocket==0 )
			{
	...
	bool DoSyncTCP()
	{
		...
		m_pTCPConnection = rkt::CreateTCPConnection(0,this,DEFAULT_PACK,NetworkUserType_User3 );
		if ( m_pTCPConnection==0 )
		{
	...
	virtual void OnTimer( unsigned long dwTimerID )
	{
        AbstractSyncClient::OnTimer(dwTimerID);
...
////////////// TCP方式连接时间同步服务器 //////////////////////////////////////////////////////
class TcpSyncClient :  public AbstractSyncClient,public IConnectionEventHandler
{
    enum 
    {
        AUTO_CONNECT_TIMER = 10,            // 断线自动连
    };
protected:
...
protected:
	...
	*/
	virtual void OnConnected( IConnection * conn )
	{
        ...
        if(m_autoConnecting)
        {
            m_pTimerAixs->KillTimer(AUTO_CONNECT_TIMER, this);
        }
	}
	...
	*/
	virtual void OnDisconnected( IConnection * conn,DWORD reason )
	{
        ...
        ErrorLn("Time sync socket is disconnected!");

        m_pTimerAixs->SetTimer(AUTO_CONNECT_TIMER, 20000, this);
	}
	...
	*/
	virtual void OnError( IConnection * conn,DWORD dwErrorCode )
	{
        ...
        ErrorLn("Time sync socket is error!");
    ...
    bool DoTestTick()
    {
        ...
        if ( !m_pTCPConnection->SendData(ob.begin(),ob.size() ) )
        {
            ErrorLn("upd socket send error" << getLastError());
            return false;
        }
    ...
    virtual void OnTimer( unsigned long dwTimerID )
    {
        ...
        else if(dwTimerID == AUTO_CONNECT_TIMER)
        {
...
{
protected:
    AbstractSyncClient *       m_pClient;

public:
    DWORD  GetTick()
    {
        return m_pClient->GetTick();
    }

    DWORD GetPing()
    {
        return m_pClient->GetPing();
    }
    ...
    bool Start( const char * strIP,unsigned short nUDPPort,unsigned short nTCPPort,TimerAxis * pAxis)
    {
        ...
        {
            m_pClient = new TcpSyncClient();
        }
        else
        {
            m_pClient = new TcpUdpSyncClient();
        }
        if(m_pClient == 0)
        {
            return false;
        ...
        return m_pClient->Start(strIP, nUDPPort, nTCPPort, pAxis);
    }
