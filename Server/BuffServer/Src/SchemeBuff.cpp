...
bool CSchemeBuff::OnSchemeLoad(SCRIPT_READER reader, const char* szFileName)
{
	...
	while(pSubElement)
	{
		...
		{
			ErrorLn("buffid不能小于等于0! buffid="<<buffschemeinfo.dwBuffID);
			return false;
		}
		...
		while(pChildElement)
		{
			...
			// [effect]
			pGrandchildElement = pChildElement->FirstChildElement("effectServer");
			if(pGrandchildElement == NULL)
			{
				return false;
			}
...
{
	CSchemeBuff* pNewInstance = NewInstance();
