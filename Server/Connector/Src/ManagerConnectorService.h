...
public:
	...
	bool Start(const char * ip, int port)
	{
		...
		m_pConnector = CreateManagerConnectorProc(this, (TimerAxis *)gServerGlobal->getTimerService()->getAxis());
		if ( m_pConnector==0 )
		{
		...
		m_pConnector->Create(MSG_ENDPOINT_SCENE, gServerGlobal->getServerID(),_GT("场景服务器"));
        ...
        case MSG_MG_REQUESTSTART_RESULT:	// 软件服务器向主服务器请求启动结果消息,调用 RequestStart()后收到
            {
                ...
                gServerGlobal->getNetMessageHandler()->handleManagerResourceResult(data, len);
            }
            break;

        case MSG_MG_SENDSCRIPTDATA:		// 收到服务器启动脚本文件数据
            {
		        gServerGlobal->getNetMessageHandler()->handleManagerResourceData(data, len);
            }
            break;
		...
		case MSG_MG_CONTROLSOFTSERVER:			// 操作软件服务器消息
			{
				...
				case MG_SoftControl_SetServerUID:	// 设定软件服务器的UID
					{
		                gServerGlobal->getNetMessageHandler()->handleManagerSetGameWorldID();
					}
					break;
				}
			}
