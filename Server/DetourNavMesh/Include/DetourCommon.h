...
#define MAX_VERT_NUM			6   // 多边形最大顶点数
...
///  @param[out]	dest	The result vector. [(x, y, z), V2V1]
///  @param[in]		v1		The base vector. [(x, y, z)]
...
/// Derives the xz-plane 2D cross product of the two vectors. (uz*vx - ux*vz)
///  @param[in]		u		The LHV vector [(x, y, z)]
///  @param[in]		v		The RHV vector [(x, y, z)]
/// @return The cross product on the xz-plane.
///
/// The vectors are projected onto the xz-plane, so the y-values are ignored.
/// 这里的有些疑问：在坐标系xoz中应该是ux*vz-uz*bx,但是这里反过来了，猜测可能在左手坐标系中，这里通过转换zox轴来计算
/// 已经通过画图通过验证
/// 
...
// If point C is between AB segment then return true, or else return false
// 判断点C是否在AB直线上
inline bool dtPointBetween(const float* A, const float* B, const float* C)
{
	// if C is in A-B, then (C[2]-A[2])/(C[0]-A[0]) = (B[2]-C[2])/(B[0]-C[0])
	float f = (C[2]-A[2])*(B[0]-C[0]) - (C[0]-A[0])*(B[2]-C[2]);
	return fabsf(f) < FLOAT_EPS;
}
...
// 假设线段AB和CD相交，若交点P存在，则保存P并返回true
bool dtInterSegmentPoint2D(const float* A, const float* B, 
						   const float* C, const float* D, float* P);

bool dtGetInterPoint2DPoly(const float* A, const float* B,
						   float* verts, int nverts, float* P);
