...
class dtCrowd
{
	...
	inline INT_PTR getAgentIndex(const dtCrowdAgent* agent) const  { return agent - m_agents; }
	const MoveRequest* getActiveMoveTarget(const INT_PTR idx) const;

	bool requestMoveTargetReplan(const INT_PTR idx, dtPolyRef ref, const float* pos);
