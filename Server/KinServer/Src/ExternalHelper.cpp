...
SSharePersonProperty CExternalHelper::getSharePerson(DWORD dwPlayerID)
{
    if (dwPlayerID == INVALID_PDBID)
    {
        return SSharePersonProperty();
    }
    IShareReceiver* pShareReceiver = gServerGlobal->getShareReceiver();
    if (pShareReceiver == nullptr)
    {
        return SSharePersonProperty();
    }

    return pShareReceiver->GetSharePerson(dwPlayerID);
}
