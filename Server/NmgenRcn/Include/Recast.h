...
#ifdef NMGENRCN_EXPORTS
#define  NMGEN_API   extern "C" __declspec(dllexport)
#else
#define  NMGEN_API   extern "C" __declspec(dllimport)
#endif
...
NMGEN_API void rcFreeCompactHeightfield(rcCompactHeightfield* chf);
...
NMGEN_API void rcFreeContourSet(rcContourSet* cset);
...
///  @ingroup recast
///  @see rcAllocPolyMesh
NMGEN_API void rcFreePolyMesh(rcPolyMesh* pmesh);
...
NMGEN_API void rcFreePolyMeshDetail(rcPolyMeshDetail* dmesh);
...
///  @param[in,out]	chf		A populated compact heightfield.
NMGEN_API bool rcMarkBoxArea(const float* bmin, const float* bmax, unsigned int areaId,
...
///  @param[in]		areaId	The area id to apply. [Limit: <= #RC_WALKABLE_AREA]
///  @param[in,out]	chf		A populated compact heightfield.
NMGEN_API bool rcMarkConvexPolyArea(const float* verts, const int nverts,
						  const float hmin, const float hmax, unsigned int areaId,
						  rcCompactHeightfield& chf, unsigned int* flagList);
...
///  @param[in]		areaId	The area id to apply. [Limit: <= #RC_WALKABLE_AREA]
///  @param[in,out]	chf	A populated compact heightfield.
NMGEN_API bool rcMarkCylinderArea(const float* pos,
						const float r, const float h, unsigned int areaId,
						rcCompactHeightfield& chf, unsigned int* flagList);
...
///  @param[in,out]	chf		A populated compact heightfield.
///  @returns True if the operation completed successfully.
NMGEN_API bool rcBuildDistanceField(rcCompactHeightfield& chf);
...
///  								be merged with larger regions. [Limit: >=0] [Units: vx] 
///  @returns True if the operation completed successfully.
NMGEN_API bool rcBuildRegions(rcCompactHeightfield& chf,
...
///  @returns True if the operation completed successfully.
NMGEN_API bool rcBuildContours(rcCompactHeightfield& chf,
...
///  @returns True if the operation completed successfully.
NMGEN_API bool rcBuildPolyMesh(rcContourSet& cset, const int nvp, rcPolyMesh& mesh);
...
///  @returns True if the operation completed successfully.
NMGEN_API bool rcBuildPolyMeshDetail(const rcPolyMesh& mesh, const rcCompactHeightfield& chf,
