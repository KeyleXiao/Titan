...
void CLogRecord::RecordMoney(DWORD dwMaster, int nTarget, int nNum, BYTE byOpType, WORD wApproach, int nTime, const char * szRemark, DWORD dwReserve )
{
    ...
    // 增加数值日记
    TrackAddNumeric(OSS_OPT_TYPE_PERSON, dwMaster, nTarget,nNum, OSS_NUMERIC_BINDTICKET, byOpType, wApproach, nTime, szRemark, dwReserve);
}
...
/**	批量保存玩家打造日记
*/
void CLogRecord::RecordActorForge( DWORD dwUserID, DWORD dwActorID, BYTE nOperateType, DWORD dwGoodID, const char * szGoodName, int nTime, const char * szForgeDesc, DWORD dwReserve )
{
	DBREQ_OSS_ACTOR_GOOD_FROGE_LOG node;
	node.dwUserID = dwUserID;       // 账号ID
	node.dwActorID = dwActorID;     // 角色ID
	node.dwGoodID = dwGoodID;		// 升级类型（1=等级，2=段位，3=英雄评级）
	sstrcpyn(node.szGoodsName,szGoodName, sizeof(node.szGoodsName));
	node.nOperateType = nOperateType;
	sstrcpyn(node.szDesc,szForgeDesc, sizeof(node.szDesc));
	node.dwOptTime = nTime;         // 发生时间
	node.dwReserveID = dwReserve;	// 保留（英雄评级时为英雄ID）

	//加入本条日志
	m_lstActorFrogeLog.push_back(node);
	int nLogCount = m_lstActorFrogeLog.size();

	size_t nSize = (nLogCount + 2)*sizeof(DBREQ_OSS_ACTOR_GOOD_FROGE_LOG) + sizeof(DBREQ_PARAM_OSS_ACTOR_GOOD_FROGE_LOGS);
	if (nSize >= 7*1024)
	{
		BatchSaveActorFrogeLog();
	}
}

// 保存玩家打造日志
void CLogRecord::BatchSaveActorFrogeLog()
{
	if (m_lstActorFrogeLog.empty())
	{
		return;
	}
	...
	obuf obLog;
	DBREQ_PARAM_OSS_ACTOR_GOOD_FROGE_LOGS log;

	size_t nSize = 0;
	obuf obData;

	// 保存单条记录
	for(TLIST_ACTOR_FROGE_LOG::iterator iter = m_lstActorFrogeLog.begin(); iter != m_lstActorFrogeLog.end(); ++iter)
	{
		if (nSize >= 7*1024)
		{
			int nLogLength = log.Num * sizeof(DBREQ_OSS_ACTOR_GOOD_FROGE_LOG);

			obLog.push_back(&log, sizeof(log));            
			obLog.push_back(&nLogLength, sizeof(nLogLength));      // BUFF长度
			obLog.push_back(obData.data(), obData.size());

			SaveToVan(GAMEDB_REQUEST_ACTOR_GOOD_FROGE_LOG, obLog.data(), obLog.size());
		...
		DBREQ_OSS_ACTOR_GOOD_FROGE_LOG & item = (*iter);
		if(item.dwOptTime == 0) item.dwOptTime = nCurTime;
		obData.push_back(&item, sizeof(item));
...
void CLogRecord::TrackAddNumeric(BYTE byObjType, int nObjectID, int nTarget, int nNum, BYTE byNumType, BYTE byOpType, WORD wApproach, int nTime, const char * szRemark, DWORD dwReserve)
{
    ...
    actorValue.dwReserveID2 = dwReserve;        // 保留ID2(战场存匹配分升降时填写战场类型）
...
void CLogRecord::SaveAllLog()
{
    ...
    // 保存所有非人物记录
    TraceLn("All Logs Save: other number.....");
    ...
    // 保存点券日记
    TraceLn("All Logs Save: people's ticket.....");
    SavePlazaTicketLog();

    // 批量保存玩家升级日记
    TraceLn("All Logs Save: people's upgrade.....");
    BatchSaveActorUpgradeLog();

    // 批量保存玩家物品日记
    TraceLn("All Logs Save: people's goods.....");
    BatchSaveActorGoodsLog();

	// 批量保存玩家打造日志
    TraceLn("All Logs Save: people's build.....");
	BatchSaveActorFrogeLog();
}
