...
bool COSSLogServer::create(void)
{
    ...
    if(pMiscManager == 0)
    {
        ErrorLn(__FUNCTION__": pMiscManager == 0");
        return false;
    }
    m_pService = pMiscManager->create_service("OSSLogService");
    if(m_pService == 0)
    {
        ErrorLn(__FUNCTION__": create service failed!");
        return false;
    }
...
@ param dwReserve   : 保留数据
*/
void COSSLogServer::addActorForgeLog( int nGameWorldID, DWORD dwUserID, DWORD dwActorID, BYTE nOperateType, DWORD dwGoodID,const char * szGoodName ,int nTime,const char * szForgeDesc, DWORD dwReserve )
{
	if(dwGoodID == 0)
	{
		return;
	...
	CLogRecord* pLogRecord = NULL;
	GETLOGRECORDBYPDBID(pLogRecord, nGameWorldID, dwActorID, __FUNCTION__);
	pLogRecord->RecordActorForge( dwUserID,  dwActorID,  nOperateType,  dwGoodID, szGoodName , nTime, szForgeDesc,  dwReserve);
...
*/
void COSSLogServer::addTaskInfo(int nTaskID, int nAcceptNum, int nFinishNum, int nGiveUpNum)
{
	m_systemMonitor.AddTaskInfo(nTaskID, nAcceptNum, nFinishNum, nGiveUpNum);
}
