...
{
	return gServerGlobal->getMessageDispatch();
}
...
void CNetMessageHandler::handleServerListUpdated()
{
    ...
    IServerGlobalService * pServerGlobalService = ServerGlobalHelper().m_ptr;
    if (pServerGlobalService)
    {
        pServerGlobalService->handleServerListUpdated();
    }
#else
...
void CNetMessageHandler::handleServerInfoUpdated(CSID ServerID,SERVER_STATE nState,ServerData * pInfo)
{
    ...
    if (pServerGlobalService)
    {
        pServerGlobalService->handleServerInfoUpdated(ServerID, nState, pInfo, sizeof(ServerData));
    }
#else
...
void CNetMessageHandler::handlePostMessage(CGID server,const char * pData,WORD wDataLen)
{
    ...
    if (pServerGlobalService)
    {
        pServerGlobalService->handlePostMessage(server, pData, wDataLen);
    }
#else
...
void CNetMessageHandler::handleReceivedData(ClientID client,LPVOID pData,DWORD wDataLen)
{
    ...
    if (pServerGlobalService)
    {
        pServerGlobalService->handleGateWayReceivedData(client, pData, wDataLen);
    }
#else
...
{
     IServerGlobalService * pGlobalService = ServerGlobalHelper().m_ptr;
    if (pGlobalService)
    {
