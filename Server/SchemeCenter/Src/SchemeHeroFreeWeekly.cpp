...
{
	CSchemeHeroFreeWeekly* pNewInstance = NewInstance();
...
@return  
*/
bool CSchemeHeroFreeWeekly::IsFreeHeroListChanged( int nLv )
{
	int lastFreeHeroArray[nFreeHeroArrayLen] = {0};
	int freeHeroArray[nFreeHeroArrayLen] = {0};
	memcpy(lastFreeHeroArray, arrayFreeHeroID, sizeof(lastFreeHeroArray));
	memset(freeHeroArray, 0, sizeof(freeHeroArray));

	int nCount = GetFreeHeroListByLv(freeHeroArray, nFreeHeroArrayLen, nLv);
	int i;
	for (i = 0; i < nCount; i++)
	{
		if (freeHeroArray[i] != lastFreeHeroArray[i])
		{
			break;
		}
	}

	if (i == nCount && lastFreeHeroArray[i] == 0)
	{
		return false;
	}
