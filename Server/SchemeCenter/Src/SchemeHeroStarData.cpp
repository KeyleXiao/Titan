...
int CSchemeHeroStarData::getUpgradeNeedExpTypeID( int nHeroId, int nStarLv)
{
    ...
    SHeroStarSchemeInfo sHeroStarInfo = itStarLv->second;
    
    return sHeroStarInfo.nExpType;
...
SHeroExclusiveTalentSchemeInfo CSchemeHeroStarData::getHeroExclusive( int nExclusiveID)
{
    EXCLUSIVE_TALENT_SCHEMEINFOMAP::iterator it = m_mapHeroExclusiveTalentInfo.find( nExclusiveID );
    if ( it == m_mapHeroExclusiveTalentInfo.end() )
    {
        return SHeroExclusiveTalentSchemeInfo();
    }
...
bool CSchemeHeroStarData::OnSchemeLoad(SCRIPT_READER reader,const char* szFileName)
{
    ...
    if ( rkt::StringHelper::casecmp( szFileName, stHeroStarPath.c_str()) == 0 )
    {
        bResult = LoadStarLvScheme(reader);
    }else if ( rkt::StringHelper::casecmp(szFileName, stHeroProperty.c_str()) == 0 )
    {
        bResult = LoadHeroPropertyScheme(reader);
    }else if ( rkt::StringHelper::casecmp( szFileName, stExclusiveTalent.c_str()) == 0)
    {
...
bool CSchemeHeroStarData::LoadStarLvScheme(SCRIPT_READER reader)
{
    ...
    for ( int i = 0; i < nCount; ++i )
    {
        ...
        HERO_STAR_SCHEMEINFOMAP::iterator itStarInfo = m_mapHeroStarSchemeInfo.find(item.nHeroID);
        if ( itStarInfo == m_mapHeroStarSchemeInfo.end() )
        {
            mapHeroStarSchemeInfo[item.nStarLV] = item;
            m_mapHeroStarSchemeInfo[item.nHeroID] = mapHeroStarSchemeInfo;
        }else
        {
...
bool CSchemeHeroStarData::LoadExclusiveTalentScheme(SCRIPT_READER reader)
{
    ...
    for ( int i = 0; i <nCount; ++i )
    {
        ...
        item.nPic = pCSVReader->GetInt(i, nColumnIndex++, 0);
