...
@return  
*/
bool CSchemeMapInfo::LoadScheme()
{
	...
	if(!bResult)
	{
		Error("CSchemeMapInfo::LoadScheme stringPath=" << stringPath.c_str() << endl);
		return false;
	}
...
bool CSchemeMapInfo::OnSchemeLoad(SCRIPT_READER reader, LPCSTR szFileName)
{
	...
	while(pSubElement)
	{
        ...
        if(pChildElement != NULL)
        {
            ...
            if(pszAttrProp != NULL)
            {
                gServerGlobal->getHeightFieldMgr()->load(mapschemeinfo.lMapID, (char*)pszAttrProp);
...
// 获取所有地图ID
void CSchemeMapInfo::getMapIDList(int* pOutArray, int& nLen)
{
    int nCount = 0;
    for(TMAP_MAPINFO::iterator it = m_mapSchemeInfo.begin(); it != m_mapSchemeInfo.end(); ++it)
    {
        pOutArray[nCount++] = it->first;
        if(nCount >= nLen)
        {
            return;
        }
