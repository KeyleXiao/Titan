...
bool CSchemeMatchRoom::LoadScheme( void )
{
	...
	if(!bResult)
	{
		Error("CSchemeMatchRoom::LoadScheme failed! filename=" << stringPath.c_str() << endl);
		return false;
	}
...
bool CSchemeMatchRoom::OnSchemeLoad( SCRIPT_READER reader, const char* szFileName )
{
	...
	for (int nRow = 0; nRow < nRecordCount; ++nRow)
	{
		...
		pData->m_NameTextInfoID		= pCSVReader->GetInt(nRow, nIndex++, 0);            // 玩法名称TextInfoID
	...
	T_Mode2Datas::iterator itor = m_Mode2Datas.begin();
...
{
	T_Mode2Datas::iterator itor = m_Mode2Datas.find( ModeID );
...
{
	T_Map2Datas::iterator itor = m_Map2Datas.find( MapID );
...
{
    T_SchemeInfo::iterator iter = m_SchemeInfo.begin();
...
CSchemeMatchRoom::T_SchemeInfo CSchemeMatchRoom::ms_NullSchemeInfo;