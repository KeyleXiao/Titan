...
{
	CSchemePersonModel* pNewInstance = NewInstance();
...
@return  
*/
BYTE CSchemePersonModel::GetHeroType(int nHeroID)
{
    TMAP_PERSONMODEL::iterator iter = m_mapPersonModel.find(nHeroID);

    return (iter!=m_mapPersonModel.end() ? iter->second.nType : 0);
}
...
//
//bool CSchemePersonModel::GetGetPersonSkinModelItemList(int nHeroID, PersonSkinModelItem* pOutArray, int& nLen)
//{
//    if(pOutArray == NULL || nLen <= 0)
//    {
//        return false;
//    }
//	TMAP_PERSONMODEL::iterator iter = m_mapPersonModel.find(nHeroID);
...
//   
//    int nCount = 0; 
//    for(std::list<PersonSkinModelItem>::iterator it = iter->second.skinModelList.begin(); 
//        it != iter->second.skinModelList.end(); ++it)
//    {
//        pOutArray[nCount++] = *it;
//        if(nCount >= nLen)
//        {
...
bool CSchemePersonModel::isHaveThisHero( int nHeroID )
{
    // 先判断配置表中是否有这种英雄
    TMAP_PERSONMODEL::iterator iter = m_mapPersonModel.find(nHeroID);

    return (iter != m_mapPersonModel.end());
}

// 获取配置英雄数量
int CSchemePersonModel::getHeroCount()
{
	return m_mapPersonModel.size();
}

// 获取所有开放英雄列表
bool CSchemePersonModel::getOpenHeroList(int *pOutArray, int &nCount)
{
	if (pOutArray == NULL || nCount <= 0)
		return false;

	int nIndex = 0;
	
	TMAP_PERSONMODEL::iterator iterEnd = m_mapPersonModel.end();
	for (TMAP_PERSONMODEL::iterator iter = m_mapPersonModel.begin(); iter != iterEnd; ++iter)
	{
        if (nIndex >= nCount)
            break;

		pOutArray[nIndex++] = iter->second.nHeroID;
	}
