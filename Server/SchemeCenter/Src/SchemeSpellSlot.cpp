...
{
	CSchemeSpellSlot* pNewInstance = NewInstance();
...
@return 技能槽位索引
*/
int CSchemeSpellSlot::getSpellSlotIndex(int nVocation, int nSpellID)
{
	SPELL_SLOT *pSpellSlot = getSpellSlot(nVocation);
	if (pSpellSlot == NULL)
	{
		return SPELL_SLOT_MAX_COUNT;
	}

	// 槽位索引
	int nSlotIndex = SPELL_SLOT_MAX_COUNT;

	// 读取配置的时候已判断nCount和nSpellID数组大小，此处不判断了
	for (int nIndex=0; nIndex<pSpellSlot->nCount; ++nIndex)
	{
		int nSlotSpellID = pSpellSlot->nSpellID[nIndex];
		if (nSlotSpellID <= 0)
		{
			continue;
		}

		if (nSlotSpellID == nSpellID)
		{
			nSlotIndex = nIndex;
			break;
		}
