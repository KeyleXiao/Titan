...
class CTalentScheme : public ISchemeTalent, public Singleton<CTalentScheme>, public ISchemeUpdateSink, public CSchemeCache<CTalentScheme>
{
public:
    // 加载天赋配置
    virtual bool Load();
    ...
    // 获取某个职 某个等级激活的天赋列表
    virtual void GetVocationTalentListByLevel(int nMatchType, int nMapID, int nVocation, int nLevel, int* pResultArray, int& nLen);
    ...
    // 获取某个天赋的天赋效果列表
    virtual void  GetEffectsOfTalent(int nTalentID, const STalentEffectBase** pArrayResult, int& nLen);
