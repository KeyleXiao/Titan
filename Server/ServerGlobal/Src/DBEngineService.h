...
#ifndef __SERVER_SERVERGLOBAL_SRC_DBENGINESERVICE_H__
#define __SERVER_SERVERGLOBAL_SRC_DBENGINESERVICE_H__
...
public:
	...
	bool Start()
	{
        ...
        sprintf_s(szLogAbsolutePath, "%s\\Log\\Log%d\\Scene", gServerGlobal->getWorkDir(), gServerGlobal->getServerID());
		char szDBLogPath[512] = { 0 };sprintf_s(szDBLogPath, "%s\\SceneDBRunLog.html", szLogAbsolutePath);
		char szPerforLog[512] = { 0 };sprintf_s(szPerforLog, "%s\\SceneDBPerforLog.html", szLogAbsolutePath);
...
#else
        void * pDataBuff = 0;
        int nDataSize = gServerGlobal->getResourceManager()->getData(RESOURCE_TYPE_DBSERVER, &pDataBuff);
        if(nDataSize == 0)
        {
            return false;
        }
		...
		{
			WarningLn(__FUNCTION__ ": pEventEngine==NULL");
		}
		else
		{
	...
	*/
    virtual void  outputQueue()
    {
        ...
        if(m_pDBEngine != NULL)
        {
            SAsynQueueStateInfo tempStateInfo[128]; int nArrayCount = 128;
...
#endif  // __SERVER_SERVERGLOBAL_SRC_DBENGINESERVICE_H__
