   ...
   virtual void  onWarClientMsg( UID uidActor,byte byKeyAction,PACKAGE_PTR msg )
    {
		...
		BUILD_MSG_CONTEXT_3( IWarService::onWarClientMsg,UID, uidActor,byte ,byKeyAction,PACKAGE_PTR::T_BAG, bag );
		m_pContainer->post_message( pMsg,nMsgLen,0,MSG_FLAG_NO_BLOCK);
    };
...
public:
	...
	virtual bool handle_message( SERVICE_MESSAGE * pMsg,rkt::obuf * resultBuf )
	{
		...
		{
			UID uidActor;byte byKeyAction;PACKAGE_PTR::T_BAG bag;
			GET_MSG_PARAM_3(UID, uidActor,byte ,byKeyAction,PACKAGE_PTR::T_BAG, bag);

			m_real_service->onWarClientMsg(uidActor,byKeyAction,bag.get());
			return true;
		}
