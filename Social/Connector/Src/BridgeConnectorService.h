...
public:
	...
	bool Start(const char * ip, int port, bool bOpenBridge)
	{
        ...
        else
        {
            ...
            string strWorldName;
            strWorldName = "";
            WORD wSenceID = gSocialGlobal->getServerID();
            m_pConnector = CreateBridgeConnectorProc((TimerAxis *)gSocialGlobal->getTimerService()->getAxis());
            if ( m_pConnector==0 )
            {
                ErrorLn(_GT("创建桥服务器连接器失败!"));
                return false;
            }
            m_pConnector->Create(dwGameWorldID,MSG_ENDPOINT_SOCIAL,wSenceID, "SocialServer", strWorldName.c_str());
