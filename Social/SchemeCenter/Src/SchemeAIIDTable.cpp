...
int CSchemeAIIDTable::getHeroAIDefault(int nBattleFieldID, int nVocation, int nRank )
{
    ...
    if (nRank > 0 && it->second.nAIRankCount > 0)
    {
        // 查找指定的AI ID
        int nAIIndex = 0;
        for(int n = 0; n < it->second.nAIRankCount; ++n)
        {
            if (nRank >= it->second.nRankList[n] )
            {
                nAIIndex++;

                if (nAIIndex >= MAX_AICONFIG_COUNT)
                {
                    nAIIndex = MAX_AICONFIG_COUNT - 1;
                }

                if (it->second.AIInfoList[nAIIndex].nAIID != -1)
                {
                    defaultID = it->second.AIInfoList[nAIIndex].nAIID;
                }
            }
        }
    }
