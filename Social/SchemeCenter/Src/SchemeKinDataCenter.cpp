...
bool CSchemeKinDataCenter::OnSchemeLoad(SCRIPT_READER reader,const char* szFileName)
{
    ...
    {
        bRet = LoadConfigScheme(reader.pCSVReader, szFileName);
    }

    if(rkt::StringHelper::casecmp(szFileName, strLevelPath.c_str()) == 0)
    {
...
{
	CSchemeKinDataCenter* pNewInstance = NewInstance();
...
/// purpose:  加载战队常量脚本
bool CSchemeKinDataCenter::LoadConfigScheme(ICSVReader * pCSVReader, const char* szFileName)
{
    ...
    // 加入战队需要的等级
    m_Config.nJoinNeedLevel = pCSVReader->GetInt(nLine++, 1, 0);
    if (m_Config.nCreateMoney < 0)
    {
        return false;
    }
    ...
    // 加入战队冷却
    m_Config.nJoinKinFreeze = pCSVReader->GetInt(nLine++, 1, 0);
    if (m_Config.nJoinKinFreeze < 0)
    {
        return false;
    }

	// 周活跃度连续{B7}周低于{B8}自动解散
	...
	// 进入卸任状态{nOutgoingDays}天后自动丢失队长职位
