...
int CSchemeMatchRankConfig::getRankKingGradeByOrderID(int eMatchTypeID, int nOrderID)
{
    if (eMatchTypeID >= MatchType_MaxID || eMatchTypeID <= MatchType_None)
    {
        return NULL;
    ...
    for (;findIter != iter->second.end(); ++findIter)
    {
        if ( findIter->second.bKingFlag &&  nOrderID <= findIter->second.nKingOrderSection)
        {
            nGrade = findIter->second.nGradeID;
            continue;
        }
