...
bool CSchemeMatchRankLimit::OnSchemeLoad(SCRIPT_READER reader,const char* szFileName)
{
    ...
    string strPunishPath = SCP_PATH_FORMAT(DMATCHROOM_SCHEME_PUNISH_FILENAME);
    ...
    if(rkt::StringHelper::casecmp(szFileName, strPunishPath.c_str()) == 0)
    {
...
{
    m_mapMatchRankLimitTimeMgr.clear();
    set<int> setBeginTrigger;
    set<int> setEndTrigger;
    ...
    for(int nRow=0; nRow<nRecordCount; ++nRow)
    {
        ...
        int beginTrigger = pCSVReader->GetInt(nRow, nCol++, 0);
        if (setBeginTrigger.find(beginTrigger) != setBeginTrigger.end())
        {
            Error(__FUNCTION__": config is not right! nDay ="<<nDay<<" nRow="<<nRow<<" nCol"<<--nCol);
            return false;
        }
        node.nDay = nDay;
        setBeginTrigger.insert(beginTrigger);
        node.nBeginTrigger = beginTrigger;
        ...
        int endTrigger = pCSVReader->GetInt(nRow, nCol++, 0);
        if (setEndTrigger.find(endTrigger) != setEndTrigger.end())
        {
            Error(__FUNCTION__": config is not right! nDay ="<<nDay<<" nRow="<<nRow<<" nCol"<<--nCol);
            return false;
        }
        setEndTrigger.insert(endTrigger);
        node.nEndTrigger = endTrigger;
        ...
        node.nBeginDesID    = pCSVReader->GetInt(nRow, nCol++, 0);
        node.nEndDesID      = pCSVReader->GetInt(nRow, nCol++, 0);
...
bool CSchemeMatchRankLimit::haveTimeLimitInfo(int nMatchTypeID)
{
    TMAP_WEEK_TIMELIMITE_MGR::iterator iter = m_mapMatchRankLimitTimeMgr.find((EMMatchType)nMatchTypeID);
    if (iter == m_mapMatchRankLimitTimeMgr.end())
    {
        return false;
    } 
...
int CSchemeMatchRankLimit::getMatchTimeLimitTriggerList(int nMatchTypeID, bool bBegin, int* nTriggerArray, int nMaxCount)
{
    TMAP_WEEK_TIMELIMITE_MGR::iterator iter = m_mapMatchRankLimitTimeMgr.find((EMMatchType)nMatchTypeID);
    if (iter == m_mapMatchRankLimitTimeMgr.end())
    {
    ...
    int n = 0;
    TMAP_WEEK_TIMELIMITE::iterator iterDay = iter->second.begin();
    for (;iterDay != iter->second.end(); ++iterDay)
    {
        for (list<SMatchRankLimitTimeSchemeInfo>::iterator iterTime = iterDay->second.begin(); iterTime != iterDay->second.end(); ++iterTime)
        {
            if (n >= nMaxCount)
            {
                return n;
            ...
            if (bBegin)
            {
                nTriggerArray[n++] = iterTime->nBeginTrigger;
            }
            else
            {
                nTriggerArray[n++] = iterTime->nEndTrigger;
            }
        }
    }
...
SMatchRankLimitTimeSchemeInfo* CSchemeMatchRankLimit::getMatchTimeLimitShemeInfo(int nMatchTypeID, bool bBegin, int nTriggerID)
{
    TMAP_WEEK_TIMELIMITE_MGR::iterator iter = m_mapMatchRankLimitTimeMgr.find((EMMatchType)nMatchTypeID);
    if (iter == m_mapMatchRankLimitTimeMgr.end())
    {
    ...
    for (;iterDay != iter->second.end(); ++iterDay)
    {
        for (list<SMatchRankLimitTimeSchemeInfo>::iterator iterTime = iterDay->second.begin(); iterTime != iterDay->second.end(); ++iterTime)
        {
            ...
            if (bBegin && nTriggerID == iterTime->nBeginTrigger)
            {
                return &(*iterTime);
            }
            
            if (!bBegin && nTriggerID == iterTime->nEndTrigger)
            {
                return &(*iterTime);
            }
