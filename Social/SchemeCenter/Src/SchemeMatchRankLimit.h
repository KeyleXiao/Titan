...
public:
    ...
    */
    ...
    /** 是否有时间限制
    */
    virtual bool haveTimeLimitInfo(int nMatchTypeID);

    /** 取得开始or结束触发器列表
    */
    virtual int getMatchTimeLimitTriggerList(int nMatchTypeID, bool bBegin, int* nTriggerArray, int nMaxCount);

    /** 获取时间限制配置
    */
    virtual SMatchRankLimitTimeSchemeInfo* getMatchTimeLimitShemeInfo(int nMatchTypeID, bool bBegin, int nTriggerID);
