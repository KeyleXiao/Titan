...
#ifndef __SOCIAL_SOCIALGLOBAL_SRC_DBENGINESERVICE_STUB_H__
#define __SOCIAL_SOCIALGLOBAL_SRC_DBENGINESERVICE_STUB_H__
...
public:
	...
	*/
	virtual bool registerHandler(IDBRetSink* pSink, const char* pszDesc)
	{
        ...
        // 创建DB代理
        IDBProxy * pProxy = gSocialGlobal ->getFramework()->create_db_proxy();
        if(pProxy == NULL)
        {
            return false;
        }
...
public:
	...
	virtual bool handle_message( SERVICE_MESSAGE * pMsg,rkt::obuf * resultBuf )
	{
		...
		ErrorLn("CDBEngineService_Stub::handle_message() can not found function name:" << function_name);
		return false;
	}
...
#endif  // __SOCIAL_SOCIALGLOBAL_SRC_DBENGINESERVICE_STUB_H__
